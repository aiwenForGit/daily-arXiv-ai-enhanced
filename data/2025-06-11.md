...
### [124] [KVmix: Gradient-Based Layer Importance-Aware Mixed-Precision Quantization for KV Cache](https://arxiv.org/abs/2506.08018)
*Fei Li,Song Liu,Weiguo Wu,Shiqiang Nie,Jinyu Wang*

Main category: cs.LG

TL;DR: KVmix是一种新型的混合精度量化方法，用于优化LLMs推理中的KV Cache内存问题，通过动态分配精度和长上下文优化策略，实现高效低内存推理。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs推理中KV Cache的高内存需求问题，避免现有方法在精度分配和长上下文任务中的不足。

Method: 基于梯度重要性分析的混合精度量化方法，动态分配层间精度，优化长上下文任务中的KV对存储。

Result: 在Llama和Mistral等LLMs上，KVmix实现了接近无损的推理性能，内存压缩4.9倍，推理速度提升5.3倍。

Conclusion: KVmix通过动态混合精度量化和长上下文优化，显著提升了LLMs在资源受限平台上的部署效率。
...
### [125] [Gridding Forced Displacement using Semi-Supervised Learning](https://arxiv.org/abs/2506.08019)
*Andrew Wells,Geraldine Henningsen,Brice Bolane Tchinde Kengne*

Main category: cs.LG

TL;DR: 半监督方法将难民统计数据从行政边界分解到0.5度网格单元，结合多种数据源实现高精度定位。
...
Method: 整合UNHCR注册数据、卫星建筑足迹和OpenStreetMap坐标，采用标签传播算法生成高精度难民分布。

Result: 平均准确率92.9%，定位1000多万难民数据，揭示局部位移模式。
...
### [126] [Bi-level Unbalanced Optimal Transport for Partial Domain Adaptation](https://arxiv.org/abs/2506.08020)
*Zi-Ying Chen,Chuan-Xian Ren,Hong Yan*

Main category: cs.LG

TL;DR: 论文提出了一种双层次不平衡最优传输（BUOT）模型，用于解决部分域自适应（PDA）问题，通过同时表征样本级和类级关系，提升知识转移的准确性。
...
Method: 提出BUOT模型，通过样本级和类级传输的合作机制，结合标签感知传输成本，优化对齐过程。

Result: 在基准数据集上的实验验证了BUOT的竞争力。

Conclusion: BUOT模型通过双层次传输框架有效解决了部分域自适应问题，提升了异常类的识别和知识转移效果。
...
### [127] [FlowBERT: Prompt-tuned BERT for variable flow field prediction](https://arxiv.org/abs/2506.08021)
*Weihao Zou,Weibing Feng,Pin Wu*
...
<details>
  <summary>Details</summary>
Motivation: 传统CFD方法计算成本高，现有深度学习模型在跨条件迁移能力上有限，亟需一种高效且通用的流场预测方法。

Method: 结合POD降维与预训练LLM微调策略，设计流体动力学导向的文本模板，提升预测性能。

Result: 在少样本学习场景中优于传统Transformer模型，跨流入条件和翼型几何表现出色，预测时间从小时级降至秒级，精度超90%。

Conclusion: 该框架为快速流体动力学预测开辟新方向，潜在应用包括气动优化、流动控制等工程领域。
...
### [128] [Modality-Balancing Preference Optimization of Large Multimodal Models by Adversarial Negative Mining](https://arxiv.org/abs/2506.08022)
*Chenxi Liu,Tianyi Xiong,Ruibo Chen,Yihan Wu,Junfeng Guo,Tianyi Zhou,Heng Huang*
...
<details>
  <summary>Details</summary>
Motivation: 现有LMMs在推理中存在模态不平衡问题，偏向语言先验而忽视视觉输入，导致下游任务泛化能力差和幻觉现象。现有偏好优化方法未有效抑制LLM内部偏见，且依赖离线数据。

Method: MBPO通过对抗性扰动生成硬负样本构建离线偏好数据集，并利用封闭式任务生成在线验证奖励数据，结合GRPO方法训练模型。

Result: 实验表明MBPO能显著提升LMMs在视觉语言任务中的性能，并有效减少幻觉。

Conclusion: MBPO通过平衡模态偏好和动态数据优化，为LMMs的模态对齐提供了有效解决方案。
...
### [129] [Recipes for Pre-training LLMs with MXFP8](https://arxiv.org/abs/2506.08027)
*Asit Mishra,Dusan Stosic,Simon Layton*

Main category: cs.LG

TL;DR: 论文探讨了在预训练中使用MXFP8格式的精度缩放技术，通过改进舍入模式解决了LLM预训练中的发散问题。


<details>
  <summary>Details</summary>
Motivation: 研究MX格式在LLM预训练中的实际应用问题，特别是舍入模式对模型收敛的影响。

Method: 提出了一种改进的舍入模式（round-to-infinity），用于计算MXFP8格式的缩放因子。

Result: 成功在15T tokens上预训练了一个8B模型，证明了改进方法的有效性。

Conclusion: 改进的舍入模式解决了MXFP8格式在LLM预训练中的发散问题，提升了精度缩放技术的实用性。
...
### [130] [ST-GraphNet: A Spatio-Temporal Graph Neural Network for Understanding and Predicting Automated Vehicle Crash Severity](https://arxiv.org/abs/2506.08051)
*Mahmuda Sultana Mimi,Md Monzurul Islam,Anannya Ghosh Tusti,Shriyank Somvanshi,Subasish Das*
...
Method: 构建两种互补的图表示：细粒度图和粗粒度图，结合多模态数据（如语义、空间和时间属性），并评估多种图神经网络架构。

Result: ST-GraphNet在粗粒度H3图上使用DSTGCN架构，测试准确率达97.74%，显著优于细粒度模型（64.7%）。
...
### [131] [STAMImputer: Spatio-Temporal Attention MoE for Traffic Data Imputation](https://arxiv.org/abs/2506.08054)
*Yiming Wang,Hao Peng,Senzhang Wang,Haohua Du,Chunyang Liu,Jia Wu,Guanlin Wu*
...
Method: 采用混合专家（MoE）框架捕捉时空特征及其权重，并设计低秩引导采样图注意力机制（LrSGAT）动态平衡路网局部与全局相关性。

Result: 在四个交通数据集上的实验表明，STAMImputer性能显著优于现有方法。

Conclusion: STAMImputer通过动态图结构和MoE框架，有效解决了交通数据填补中的关键问题。
...
### [132] [Eliciting Fine-Tuned Transformer Capabilities via Inference-Time Techniques](https://arxiv.org/abs/2506.08060)
*Asankhaya Sharma*
...
<details>
  <summary>Details</summary>
Motivation: 减少监督微调的计算成本，探索大型语言模型在资源有限情况下的高效部署。

Method: 利用上下文学习（ICL）技术，在推理时近似监督微调的能力，分析不同任务所需的数据集规模。

Result: 在文本生成和线性分类任务中，确定了近似微调行为所需的数据集规模，并提供了理论支持。

Conclusion: 为大型语言模型的高效部署提供了理论基础，实际技术如检索增强生成可连接理论与应用。
...
### [133] [FairDICE: Fairness-Driven Offline Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2506.08062)
*Woosung Kim,Jinho Lee,Jongmin Lee,Byung-Jun Lee*
...
<details>
  <summary>Details</summary>
Motivation: 现有线性标量化方法无法捕捉公平导向目标（如Nash社会福利或最大最小公平），离线环境下非线性福利标准的统一优化方法尚未探索。

Method: FairDICE利用分布校正估计联合考虑福利最大化和分布正则化，实现稳定且样本高效的学习。

Result: 在多个离线基准测试中，FairDICE表现出优于现有基线的公平感知性能。

Conclusion: FairDICE为离线MORL中非线性福利目标的优化提供了有效解决方案。
...
### [134] [Lite-RVFL: A Lightweight Random Vector Functional-Link Neural Network for Learning Under Concept Drift](https://arxiv.org/abs/2506.08063)
*Songqiao Hu,Zeyi Liu,Xiao He*

Main category: cs.LG

TL;DR: 提出了一种轻量级快速高效的随机向量功能链接网络Lite-RVFL，无需检测概念漂移或重新训练即可适应数据分布变化。


<details>
  <summary>Details</summary>
Motivation: 在线学习方法在数据分布随时间变化（概念漂移）时可靠性下降，现有方法计算成本高且不适合实时应用。

Method: Lite-RVFL通过指数加权新样本的目标函数，强调近期数据并实现快速适应，同时推导了高效的增量更新规则。

Result: 实验验证了Lite-RVFL在适应概念漂移和捕捉时间模式上的高效性和有效性。

Conclusion: Lite-RVFL是一种轻量级、快速且高效的解决方案，适用于实时应用中的概念漂移问题。
...
### [135] [Info-Coevolution: An Efficient Framework for Data Model Coevolution](https://arxiv.org/abs/2506.08070)
*Ziheng Qin,Hailun Xu,Wei Chee Yew,Qi Jia,Yang Luo,Kanchan Sarkar,Danhui Guan,Kai Wang,Yang You*
...
<details>
  <summary>Details</summary>
Motivation: 现实世界数据的持续增长导致数据集构建和训练效率低下，传统方法无法高效选择需要标注的新数据。

Method: 利用任务特定模型和开源模型，选择性标注并整合在线和网络数据，实现高效数据集改进。

Result: 在ImageNet-1K等数据集上，标注和训练成本降低32%，且无需调参即可自动确定节省比例。结合半监督学习，标注比例可进一步降至50%。

Conclusion: Info-Coevolution通过无偏见的在线选择性标注，显著提升了数据效率和训练效果。
...
### [136] [Benchmarking Pre-Trained Time Series Models for Electricity Price Forecasting](https://arxiv.org/abs/2506.08113)
*Timothée Hornek Amir Sartipi,Igor Tchappi,Gilbert Fridgen*
...
<details>
  <summary>Details</summary>
Motivation: 电力价格预测对电力现货市场交易决策至关重要，但生成式人工智能和时间序列基础模型在EPF中的效果尚不明确，因此需要评估其性能。

Method: 使用德国、法国、荷兰、奥地利和比利时的2024年日前拍卖电力价格数据，比较了多种TSFMs（如Chronos-Bolt、Time-MoE等）与传统方法（如MSTL模型）的预测效果。

Result: Chronos-Bolt和Time-MoE在TSFMs中表现最佳，与传统方法相当，但双季节性MSTL模型在一致性和性能上更优。

Conclusion: TSFMs在EPF中表现与传统方法相当，但MSTL模型因其捕获季节性的能力而成为更可靠的选择。
...
### [137] [Bingo: Boosting Efficient Reasoning of LLMs via Dynamic and Significance-based Reinforcement Learning](https://arxiv.org/abs/2506.08125)
*Hanbing Liu,Lang Cao,Yuanyi Ren,Mengyu Zhou,Haoyu Dong,Xiaojun Ma,Shi Han,Dongmei Zhang*
...
Method: Bingo引入两种机制：显著性感知长度奖励和动态长度奖励，逐步减少冗余输出并优化效率。

Result: 实验表明，Bingo在多个推理基准上优于基线方法，实现了准确性与效率的平衡。
...
### [138] [Nearness of Neighbors Attention for Regression in Supervised Finetuning](https://arxiv.org/abs/2506.08139)
*Aviad Susman,Mayte Suárez-Fariñas,Joseph T Colonel*

Main category: cs.LG

TL;DR: 论文提出了一种名为NONA的可微分k-NN回归层，通过结合神经网络注意力和学习掩码机制，提升了回归任务的性能。


<details>
  <summary>Details</summary>
Motivation: 传统算法（如k-NN）在监督微调（SFT）后的特征提取中表现优异，但由于不可微分性难以直接融入神经网络。

Method: 提出NONA层，利用注意力机制和掩码方案模拟k-NN回归，使其可微分并适用于神经网络。

Result: 在多个非结构化数据集上，NONA的性能优于传统的密集层预测和k-NN方法。
...
### [139] [AutoSDT: Scaling Data-Driven Discovery Tasks Toward Open Co-Scientists](https://arxiv.org/abs/2506.08140)
*Yifei Li,Hanane Nour Moussa,Ziru Chen,Shijie Chen,Botao Yu,Mingyi Xue,Benjamin Burns,Tzu-Yao Chiu,Vishal Dey,Zitong Lu,Chen Wei,Qianheng Zhang,Tianyu Zhang,Song Gao,Xuhui Huang,Xia Ning,Nesreen K. Ahmed,Ali Payani,Huan Sun*

Main category: cs.LG

TL;DR: AutoSDT是一个自动收集高质量编码任务的管道，用于解决数据稀缺问题，构建了AutoSDT-5K数据集，并训练了AutoSDT-Coder模型，显著提升了数据驱动科学发现的性能。


<details>
  <summary>Details</summary>
Motivation: 加速科学发现需要高质量的AI辅助工具，但现有数据稀缺且质量不足，阻碍了AI模型的训练与评估。

Method: 利用LLM的编码能力和参数知识，自动搜索多样来源、选择生态有效任务，并合成任务指令和代码解决方案，构建AutoSDT-5K数据集。

Result: AutoSDT-5K数据集包含5,404个任务，专家评估显示93%的任务生态有效，92.2%的代码功能正确。AutoSDT-Coder模型在多个基准测试中表现优异，接近GPT-4o水平。

Conclusion: AutoSDT解决了数据稀缺问题，显著提升了AI在科学发现中的性能，为未来研究提供了高质量数据集和模型。
...
### [140] [Accelerating Spectral Clustering under Fairness Constraints](https://arxiv.org/abs/2506.08143)
*Francesco Tonin,Alex Lambert,Johan A. K. Suykens,Volkan Cevher*

Main category: cs.LG

TL;DR: 提出了一种高效的公平谱聚类方法（Fair SC），通过将问题转化为凸差函数（DC）框架，并引入变量增强策略和ADMM算法，显著提高了计算效率。


<details>
  <summary>Details</summary>
Motivation: 解决决策算法中的公平性问题，确保每个聚类中人口统计群体的比例与总体一致。

Method: 将公平谱聚类问题转化为DC框架，采用变量增强策略和ADMM算法，避免昂贵的特征分解。

Result: 在合成和真实数据集上验证了方法的有效性，计算时间显著优于现有方法。
...
### [141] [Fully data-driven inverse hyperelasticity with hyper-network neural ODE fields](https://arxiv.org/abs/2506.08146)
*Vahidullah Taç,Amirhossein Amiri-Hezaveh,Manuel K. Rausch,Grace N. Bechtel,Francisco Sahli Costabal,Adrian Buganza Tepole*

Main category: cs.LG

TL;DR: 提出了一种新框架，通过神经网络和物理驱动的数据驱动方法识别异质材料的力学特性，无需封闭形式的本构方程。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要封闭形式的本构方程，限制了异质材料力学特性的识别。新框架旨在克服这一限制。

Method: 结合傅里叶特征的神经网络近似应变场，利用NODE框架发现本构方程，并通过超网络处理异质性。

Result: 数值实验表明该方法在识别异质材料力学特性时具有鲁棒性和通用性。

Conclusion: 该框架为识别异质材料力学特性提供了有效的替代方案，适用于实验数据。
...
### [142] [BLUR: A Bi-Level Optimization Approach for LLM Unlearning](https://arxiv.org/abs/2506.08164)
*Hadi Reisizadeh,Jinghan Jia,Zhiqi Bu,Bhanukiran Vinzamuri,Anil Ramakrishna,Kai-Wei Chang,Volkan Cevher,Sijia Liu,Mingyi Hong*

Main category: cs.LG

TL;DR: 论文提出了一种新的遗忘学习问题分层建模方法，并设计了Bi-Level UnleaRning (BLUR)算法，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 确保大语言模型(LLM)在训练后能够遗忘特定知识或能力，以符合数据法规和伦理要求。

Method: 采用双层优化框架，下层最小化遗忘损失，上层保持模型效用，提出BLUR算法。

Result: BLUR在多种任务、模型和指标上均优于现有方法。

Conclusion: 分层建模和BLUR算法为LLM遗忘学习提供了有效解决方案。
...
### [143] [UniVarFL: Uniformity and Variance Regularized Federated Learning for Heterogeneous Data](https://arxiv.org/abs/2506.08167)
*Sunny Gupta,Nikita Jangid,Amit Sethi*
...
Method: 提出UniVarFL框架，结合分类器方差正则化和超球面均匀性正则化，模拟IID训练动态。
...
### [144] [Federated Learning on Stochastic Neural Networks](https://arxiv.org/abs/2506.08169)
*Jingqiao Tang,Ryan Bausback,Feng Bao,Richard Archibald*

Main category: cs.LG

TL;DR: 提出了一种基于随机神经网络的联邦学习方法，用于处理本地数据中的潜在噪声。
...
Method: 在联邦学习框架中使用随机神经网络作为本地模型，估计数据真实状态并量化噪声。

Result: 数值实验表明该方法在处理非独立同分布数据时表现优异。

Conclusion: 联邦随机神经网络能有效解决本地数据噪声问题，提升模型鲁棒性。
...
### [145] [FedGA-Tree: Federated Decision Tree using Genetic Algorithm](https://arxiv.org/abs/2506.08176)
*Anh V Nguyen,Diego Klabjan*
...
<details>
  <summary>Details</summary>
Motivation: 当前联邦学习主要关注基于梯度的参数模型，非参数模型（如决策树）研究较少，且现有方法受限于差分隐私，仅适用于分类任务和分类数据。

Method: 利用遗传算法构建个性化的决策树，支持分类和数值数据，适用于分类和回归任务。

Result: 实验表明，该方法优于仅基于本地数据训练的决策树和基准算法。

Conclusion: 遗传算法为联邦学习中的决策树提供了一种灵活且高效的方法，扩展了其应用范围。
...
### [146] [Correlated Noise Mechanisms for Differentially Private Learning](https://arxiv.org/abs/2506.08201)
*Krishna Pillutla,Jalaj Upadhyay,Christopher A. Choquette-Choo,Krishnamurthy Dvijotham,Arun Ganesh,Monika Henzinger,Jonathan Katz,Ryan McKenna,H. Brendan McMahan,Keith Rush,Thomas Steinke,Abhradeep Thakurta*
...
<details>
  <summary>Details</summary>
Motivation: 传统DP机制在随机梯度下降（SGD）的每一步注入独立噪声以保护训练数据的隐私，但研究表明，通过引入（反）相关噪声可以显著改善隐私与效用的权衡。

Method: 研究采用加权前缀和估计作为核心方法，探讨了矩阵机制、因子分解机制和DP-FTRL等噪声机制。

Result: 相关噪声机制在实践中具有重要影响，已在全球范围内实现工业级部署。

Conclusion: 相关噪声机制能够通过噪声抵消优化隐私-效用权衡，为DP在AI训练中的应用提供了新思路。
...
### [147] [A Machine Learning Approach to Generate Residual Stress Distributions using Sparse Characterization Data in Friction-Stir Processed Parts](https://arxiv.org/abs/2506.08205)
*Shadab Anwar Shaikh,Kranthi Balusu,Ayoub Soulami*

Main category: cs.LG

TL;DR: 论文提出了一种基于机器学习的残余应力生成器（RSG），用于从有限测量数据推断全场应力分布，显著减少实验工作量。


<details>
  <summary>Details</summary>
Motivation: 残余应力会影响组件性能，但全场应力分布的实验表征不切实际，因此需要一种高效的方法。

Method: 通过大量工艺模拟构建数据集，使用U-Net架构的机器学习模型进行训练和超参数调优。

Result: 模型在模拟应力预测中表现出高精度和泛化能力，实验数据验证了其有效性。

Conclusion: RSG方法可行，能从有限测量中全面理解残余应力分布，显著降低实验成本。
...
### [148] [What makes an Ensemble (Un) Interpretable?](https://arxiv.org/abs/2506.08216)
*Shahaf Bassan,Guy Amir,Meirav Zehavi,Guy Katz*

Main category: cs.LG

TL;DR: 论文探讨了集成模型的解释性问题，通过计算复杂性理论分析了影响解释性的因素，发现不同配置的集成模型在解释性上存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 集成模型（如提升树）通常被视为黑箱，缺乏对其解释性为何受限的严格数学理解。本文旨在填补这一空白。

Method: 应用计算复杂性理论，研究不同因素（如基础模型的数量、大小和类型）对集成模型解释性的影响。

Result: 研究发现，即使基础模型大小固定，解释集成模型仍具有计算难度；而基础模型数量对解释性影响显著。

Conclusion: 通过计算复杂性视角，为理解集成模型的解释性提供了更坚实的基础，强调了这一方法的优势。
...
### [149] [Mondrian: Transformer Operators via Domain Decomposition](https://arxiv.org/abs/2506.08226)
*Arthur Feeney,Kuei-Hsiang Huang,Aparna Chandramowlishwaran*

Main category: cs.LG

TL;DR: Mondrian提出了一种基于域分解的Transformer算子，通过将域划分为非重叠子域并应用注意力机制，解决了高分辨率多尺度PDE建模中的计算成本问题。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer在建模高分辨率多尺度PDE时，由于注意力机制的二次计算成本和与离散化的耦合，难以扩展。Mondrian旨在通过域分解解决这一问题。

Method: Mondrian将域分解为子域，在每个子域内使用神经算子替代标准层，并通过基于softmax的内积计算子域间的注意力。支持分层窗口和邻域注意力。

Result: 在Allen-Cahn和Navier-Stokes PDE上表现优异，展示了无需重新训练的分辨率扩展能力。
...
### [150] [Scaling Laws of Motion Forecasting and Planning -- A Technical Report](https://arxiv.org/abs/2506.08228)
*Mustafa Baniodeh,Kratarth Goel,Scott Ettinger,Carlos Fuertes,Ari Seff,Tim Shen,Cole Gulino,Chenjie Yang,Ghassen Jerfel,Dokook Choe,Rui Wang,Vinutha Kallem,Sergio Casas,Rami Al-Rfou,Benjamin Sapp,Dragomir Anguelov*

Main category: cs.LG

TL;DR: 论文研究了自动驾驶领域中编码器-解码器自回归Transformer模型的缩放规律，发现模型性能随计算预算呈幂律提升，并探讨了训练与推理时的最优缩放策略。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索在自动驾驶任务中，如何通过缩放模型和数据集规模来优化性能，并验证闭环指标与开环指标的相关性。

Method: 使用50万小时驾驶数据集，分析模型性能与计算预算的关系，研究模型参数与数据量的最优缩放比例，并探讨推理时的计算效率。

Result: 结果显示，模型性能随计算预算幂律提升，训练时模型规模需以数据量1.5倍的速度增长；推理时小模型通过采样和聚类可媲美大模型，直到交叉点后大模型更高效。

Conclusion: 优化训练和推理的缩放策略是提升自动驾驶模型性能的关键，同时利用其他车辆的日志数据可缓解机器人数据稀缺问题。
...
### [151] [Ensuring Reliability of Curated EHR-Derived Data: The Validation of Accuracy for LLM/ML-Extracted Information and Data (VALID) Framework](https://arxiv.org/abs/2506.08231)
*Melissa Estevez,Nisha Singh,Lauren Dyson,Blythe Adamson,Qianyu Yuan,Megan W. Hildner,Erin Fidyk,Olive Mbah,Farhad Khan,Kathi Seidl-Rathkopf,Aaron B. Cohen*

Main category: cs.LG

TL;DR: 论文提出一个评估LLM提取临床数据质量的综合框架，解决现有方法未覆盖的独特问题和复杂性。


<details>
  <summary>Details</summary>
Motivation: LLM在临床数据提取中的应用虽高效但存在可靠性、准确性和公平性挑战，现有质量保证框架未能完全解决。

Method: 框架整合变量级性能基准测试、自动验证检查（内部一致性和合理性）以及复制分析（对比人工提取数据或外部标准）。

Result: 多维方法能识别需改进的变量、系统性检测潜在错误，并确认数据集在真实研究中的适用性。

Conclusion: 该框架为LLM提取的RWD提供严格透明的评估方法，推动行业标准并支持AI在肿瘤学研究中的可信应用。
...
### [152] [Dealing with the Evil Twins: Improving Random Augmentation by Addressing Catastrophic Forgetting of Diverse Augmentations](https://arxiv.org/abs/2506.08240)
*Dongkyu Cho,Rumi Chunara*

Main category: cs.LG

TL;DR: 本文探讨了随机数据增强的不足，并提出了一种简单方法来解决其导致的特征扭曲问题，从而提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: 随机数据增强成本低但效果有限，而目标增强成本高但效果好。本文旨在解决随机增强的不足。

Method: 通过分析随机增强的随机性导致特征扭曲的问题，提出了一种解决遗忘效应的简单方法。

Result: 该方法在单源域泛化（sDG）基准测试中表现出强大的泛化性能。

Conclusion: 改进后的随机增强方法能够有效提升泛化效果，且成本低廉。
...
### [153] [Temporalizing Confidence: Evaluation of Chain-of-Thought Reasoning with Signal Temporal Logic](https://arxiv.org/abs/2506.08243)
*Zhenjiang Mao,Artem Bisliouk,Rohith Reddy Nama,Ivan Ruchkin*

Main category: cs.LG

TL;DR: 本文提出了一种基于信号时序逻辑（STL）的框架，用于评估大语言模型（LLMs）在数学推理任务中的逐步置信度，并通过不确定性重塑策略提升其可靠性。


<details>
  <summary>Details</summary>
Motivation: LLMs在数学推理任务中表现优异，但容易产生高置信度却错误的输出，这在教育等领域存在风险。

Method: 利用STL定义时间约束，计算鲁棒性分数作为置信度估计，并引入不确定性重塑策略。

Result: 实验表明，该方法显著改善了校准指标，提供了比传统方法更可靠的置信度估计。

Conclusion: 提出的框架有效提升了LLMs在数学推理中的置信度可靠性，具有实际应用价值。
...
### [154] [Parameter-free approximate equivariance for tasks with finite group symmetry](https://arxiv.org/abs/2506.08244)
*Riccardo Ali,Pietro Liò,Jamie Vicary*

Main category: cs.LG

TL;DR: 提出一种零参数的近似等变性方法，通过在损失函数中添加额外项，实验表明其性能优于现有方法且参数更少。


<details>
  <summary>Details</summary>
Motivation: 现有等变性方法计算复杂、参数多且架构受限，需改进。

Method: 在潜在表示中为有限群添加近似等变性损失项，网络学习潜在空间的群表示。

Result: 实验表明网络倾向于学习正则表示，性能与现有方法相当或更好，参数更少。

Conclusion: 提出的零参数方法简单有效，适用于多种任务，性能优越。
...
### [155] [SHIELD: Secure Hypernetworks for Incremental Expansion Learning Defense](https://arxiv.org/abs/2506.08255)
*Patryk Krukowski,Łukasz Gorczyca,Piotr Helm,Kamil Książek,Przemysław Spurek*

Main category: cs.LG

TL;DR: SHIELD结合超网络和区间算术，解决了持续学习中的灾难性遗忘和对抗攻击问题。
...
Method: SHIELD通过超网络生成任务嵌入向量，动态为目标模型分配权重，并结合区间算术提供严格的安全保证。

Result: SHIELD能在持续学习中动态生成子任务网络，同时抵御对抗攻击，增强安全性。

Conclusion: SHIELD在保持网络适应性的同时解决了持续学习中的安全问题。
...
### [156] [Reinforcement Learning from Human Feedback with High-Confidence Safety Constraints](https://arxiv.org/abs/2506.08266)
*Yaswanth Chittepu,Blossom Metevier,Will Schwarzer,Austin Hoag,Scott Niekum,Philip S. Thomas*

Main category: cs.LG

TL;DR: HC-RLHF是一种新方法，通过高置信度安全强化学习确保语言模型在敏感领域的安全性，同时最大化实用性。


<details>
  <summary>Details</summary>
Motivation: 现有方法常将安全性与实用性视为权衡，导致敏感领域出现不可接受的响应。HC-RLHF旨在提供高置信度安全保证。

Method: HC-RLHF将人类偏好分解为实用性和安全性，分别训练奖励模型和成本模型。通过两步优化过程（悲观约束优化和安全测试）确保安全性。

Result: 实验表明，HC-RLHF能以高概率生成安全模型，并在无害性和实用性上优于现有方法。

Conclusion: HC-RLHF为语言模型对齐提供了一种可靠的安全解决方案，兼具理论保障和实际效果。
...
### [157] [Sparse Interpretable Deep Learning with LIES Networks for Symbolic Regression](https://arxiv.org/abs/2506.08267)
*Mansooreh Montazerin,Majd Al Aawar,Antonio Ortega,Ajitesh Srivastava*
...
Method: 采用LIES架构，结合过采样策略和定制损失函数，优化稀疏性和梯度稳定性，训练后进一步剪枝简化表达式。

Result: 在基准测试中，LIES生成的符号公式稀疏且准确，优于所有基线方法。
...
### [158] [SWAT-NN: Simultaneous Weights and Architecture Training for Neural Networks in a Latent Space](https://arxiv.org/abs/2506.08270)
*Zitong Huang,Mansooreh Montazerin,Ajitesh Srivastava*

Main category: cs.LG

TL;DR: 提出一种同时优化神经网络结构和权重的新方法，通过连续潜在空间嵌入和梯度下降联合优化。


<details>
  <summary>Details</summary>
Motivation: 传统设计神经网络的方法（手动试错或NAS）效率低且分离了结构搜索和权重优化，需要一种更高效且联合优化的方法。

Method: 训练一个多尺度自编码器，将结构和参数信息嵌入连续潜在空间，通过梯度下降联合优化结构和权重，并加入稀疏性和紧凑性惩罚。

Result: 在合成回归任务中，该方法能有效发现性能优越的稀疏紧凑神经网络。

Conclusion: 该方法为神经网络设计提供了一种高效且联合优化的新思路。
...
### [159] [Universal Differential Equations for Scientific Machine Learning of Node-Wise Battery Dynamics in Smart Grids](https://arxiv.org/abs/2506.08272)
*Tarushri N. S.*
...
Method: 提出基于UDE的方法，将神经网络残差嵌入物理电池ODE中，学习节点特异性电池演化。

Result: 实验表明UDE模型与真实电池轨迹高度吻合，收敛平滑且长期预测稳定。

Conclusion: UDE方法适用于分散式能源网络电池建模，对实时控制和优化具有广泛意义。
...
### [160] [The Impact of Feature Scaling In Machine Learning: Effects on Regression and Classification Tasks](https://arxiv.org/abs/2506.08274)
*João Manoel Herrera Pinheiro,Suzana Vilas Boas de Oliveira,Thiago Henrique Segreto Silva,Pedro Antonio Rabelo Saraiva,Enzo Ferreira de Souza,Leonardo André Ambrosio,Marcelo Becker*
...
Conclusion: 研究为实践者提供了模型特定的缩放技术选择指南，并公开了所有代码和结果以确保透明性和可复现性。
...
### [161] [From Debate to Equilibrium: Belief-Driven Multi-Agent LLM Reasoning via Bayesian Nash Equilibrium](https://arxiv.org/abs/2506.08292)
*Xie Yi,Zhanke Zhou,Chentao Cao,Qiyu Niu,Tongliang Liu,Bo Han*
...
Method: 将多LLM协调建模为不完全信息博弈，采用贝叶斯纳什均衡，结合分布式推理与集中输出。

Result: ECON在六个基准测试中平均性能提升11.2%，并具有灵活扩展性。

Conclusion: ECON为更强大、可扩展的多LLM集成提供了有效解决方案。
...
### [162] [From Passive to Active Reasoning: Can Large Language Models Ask the Right Questions under Incomplete Information?](https://arxiv.org/abs/2506.08295)
*Zhanke Zhou,Xiao Feng,Zhaocheng Zhu,Jiangchao Yao,Sanmi Koyejo,Bo Han*
...
Result: 当前LLM在主动推理中表现不佳，难以获取或利用所需信息，且改进策略效果有限。

Conclusion: 需发展主动推理方法，如交互式学习、实时反馈和环境感知目标训练。
...
### [163] [H$^2$GFM: Towards unifying Homogeneity and Heterogeneity on Text-Attributed Graphs](https://arxiv.org/abs/2506.08298)
*Trung-Kien Nguyen,Heng Ping,Shixuan Li,Peiyu Zhang,Nikos Kanakaris,Nicholas Kotov,Paul Bogdan*
...
Method: H$^2$GFM通过统一的文本空间投影多样化的元关系，并利用上下文编码捕捉空间和高阶语义关系。采用上下文自适应图变换器（CGT）和专家混合模型处理异质性。

Result: 在广泛的HoTAGs和HeTAGs数据集及学习场景中，模型表现出色。

Conclusion: H$^2$GFM有效扩展了GFM的能力，适用于多种图类型和任务。
...
### [164] [Learnable Spatial-Temporal Positional Encoding for Link Prediction](https://arxiv.org/abs/2506.08309)
*Katherine Tieu,Dongqi Fu,Zihao Li,Ross Maciejewski,Jingrui He*
...
<details>
  <summary>Details</summary>
Motivation: 现有图深度学习中的位置编码方法存在适应性差、无法处理动态图信息以及计算效率低的问题，亟需改进。

Method: 提出L-STEP模型，通过可学习的时空位置编码方案，结合MLP实现高效表达，并在不同初始编码输入下验证其鲁棒性。

Result: L-STEP在13个经典数据集和10种算法中表现出色，计算效率优于现有方法，并在TGB基准测试中领先。

Conclusion: L-STEP是一种高效、适应性强的时空位置编码方法，显著提升了图深度学习模型的性能。
...
### [165] [Private Evolution Converges](https://arxiv.org/abs/2506.08312)
*Tomás González,Giulia Fanti,Aaditya Ramdas*

Main category: cs.LG

TL;DR: 论文提出了一种新的理论框架来解释Private Evolution (PE)的实际行为，并证明了其在生成差分隐私合成数据时的收敛性。


<details>
  <summary>Details</summary>
Motivation: PE在某些领域（如图像和文本）表现良好，但在其他领域（如表格数据）表现不一致。现有理论分析依赖于不现实的假设，因此需要更准确的理论解释。

Method: 作者开发了一个新的理论框架，分析了PE在d维数据集上的收敛性，并证明了其生成(ε, δ)-DP合成数据的能力。

Result: 证明了PE在n→∞时的最坏情况收敛性，并扩展到一般Banach空间。同时将PE与Private Signed Measure Mechanism联系起来。

Conclusion: 理论分析通过模拟验证了其实际意义，为PE的应用提供了更坚实的理论基础。
...
### [166] [Why Masking Diffusion Works: Condition on the Jump Schedule for Improved Discrete Diffusion](https://arxiv.org/abs/2506.08316)
*Alan N. Amin,Nate Gruver,Andrew Gordon Wilson*

Main category: cs.LG

TL;DR: 离散扩散模型通过逐步去噪生成高质量样本，但实践中表现最佳的是掩码扩散模型。研究发现掩码扩散利用了离散马尔可夫过程的跳跃特性，提出了一种新方法SCUD，能超越掩码扩散的性能。


<details>
  <summary>Details</summary>
Motivation: 探索离散扩散模型性能差异的原因，并提出改进方法。

Method: 提出SCUD方法，将跳跃时间分布融入离散扩散模型，结合归纳偏置应用于图像、文本和蛋白质数据。

Result: SCUD模型在多个任务上表现优于掩码扩散。

Conclusion: SCUD方法通过利用离散马尔可夫过程的特性，显著提升了离散扩散模型的性能。
...
### [167] [Graph Prompting for Graph Learning Models: Recent Advances and Future Directions](https://arxiv.org/abs/2506.08326)
*Xingbo Fu,Zehong Wang,Zihan Chen,Jiazheng Li,Yaochen Zhu,Zhenyu Lei,Cong Shen,Yanfang Ye,Chuxu Zhang,Jundong Li*
...
<details>
  <summary>Details</summary>
Motivation: 图学习模型在大规模图数据中表现出色，但如何通过图提示技术高效适应下游任务仍是一个关键问题。

Method: 介绍了图预训练方法作为基础，并详细阐述了图提示技术中可学习提示的设计方法。

Result: 总结了图提示技术在不同领域的实际应用。

Conclusion: 讨论了现有研究的挑战，并提出了未来研究方向。
...
### [168] [A Simple Analysis of Discretization Error in Diffusion Models](https://arxiv.org/abs/2506.08337)
*Juhyeok Choi,Chenglin Fan*

Main category: cs.LG

TL;DR: 本文提出了一种简化的理论框架，用于分析DDPM中VP-SDE的Euler-Maruyama离散化误差，并证明了在Lipschitz假设下收敛速率为$\mathcal{O}(1/T^{1/2})$。此外，研究表明离散噪声可替代高斯噪声而不影响收敛性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型的离散化误差分析常依赖复杂概率工具，本文旨在简化分析框架并探索离散噪声的实用性。

Method: 利用Grönwall不等式推导收敛速率，并实验验证离散噪声（如Rademacher或均匀噪声）的有效性。

Result: 实验验证了误差按预测比例缩放，离散噪声与高斯噪声样本质量相当，噪声缩放不当会降低性能。
...
### [169] [Dynamical System Optimization](https://arxiv.org/abs/2506.08340)
*Emo Todorov*

Main category: cs.LG

TL;DR: 论文提出了一种优化框架，通过将控制权转移给策略，从而在自治动态系统层面简化算法，避免了传统动态规划和强化学习的复杂性。


<details>
  <summary>Details</summary>
Motivation: 传统动态规划和强化学习方法复杂且计算量大，作者希望通过直接在策略参数层面优化，简化算法并统一处理多种问题。

Method: 将控制权转移给策略，形成自治动态系统，并在系统层面设计优化算法，避免直接使用动态规划和强化学习工具。

Result: 提出的算法能够计算与策略梯度、Hessian矩阵、自然梯度等相同的量，并适用于行为克隆、机制设计、系统辨识等问题。

Conclusion: 该框架不仅简化了优化过程，还统一了策略参数和其他系统参数的优化方法，适用于生成式AI模型的调优。
...
### [170] [Differentially Private Relational Learning with Entity-level Privacy Guarantees](https://arxiv.org/abs/2506.08347)
*Yinan Huang,Haoteng Ying,Eli Chien,Rongzhe Wei,Pan Li*

Main category: cs.LG

TL;DR: 论文提出了一种针对关系数据的差分隐私学习框架，解决了传统DP-SGD在关系学习中面临的敏感性和隐私放大问题。


<details>
  <summary>Details</summary>
Motivation: 在敏感领域中保护个体隐私的需求日益增长，但直接应用DP-SGD于关系学习存在挑战，如高敏感性和多阶段采样。

Method: 通过严格的敏感性分析和自适应梯度裁剪方案，扩展隐私放大结果，提出了一种专为关系数据设计的DP-SGD变体。
...
### [171] [An Adaptive Method Stabilizing Activations for Enhanced Generalization](https://arxiv.org/abs/2506.08353)
*Hyunseok Seung,Jaewoo Lee,Hyunsuk Ko*
...
<details>
  <summary>Details</summary>
Motivation: 传统激活正则化方法存在局限性，AdaAct旨在通过神经元级别的自适应优化弥补这一不足。

Method: AdaAct通过动态调整学习率以适应激活方差，结合Adam的快速收敛和SGD的强泛化能力。

Result: 在CIFAR和ImageNet等标准图像分类任务中，AdaAct表现出与现有方法竞争的性能。

Conclusion: AdaAct成功平衡了收敛速度和泛化能力，同时保持高效执行时间。
...
### [172] [NysAct: A Scalable Preconditioned Gradient Descent using Nystrom Approximation](https://arxiv.org/abs/2506.08360)
*Hyunseok Seung,Jaewoo Lee,Hyunsuk Ko*

Main category: cs.LG

TL;DR: NysAct是一种可扩展的一阶梯度预处理方法，平衡了一阶和二阶优化方法的优势，通过近似激活协方差矩阵降低计算和内存成本，同时提高测试精度。
...
### [173] [AlphaFold Database Debiasing for Robust Inverse Folding](https://arxiv.org/abs/2506.08365)
*Cheng Tan,Zhenxiao Cao,Zhangyang Gao,Siyuan Li,Yufei Huang,Stan Z. Li*

Main category: cs.LG

TL;DR: AlphaFold数据库（AFDB）的结构数据虽然覆盖广且精度高，但在训练对原子几何敏感的深度学习模型（如逆折叠）时存在系统性几何偏差。为解决这一问题，研究者提出了一种去偏结构自编码器（DeSAE），通过重构自然构象来提升模型性能。


<details>
  <summary>Details</summary>
Motivation: AFDB的结构数据虽然高质量，但其几何偏差限制了在逆折叠等任务中的泛化能力。实验结构（PDB）更具多样性，因此需要一种方法消除AFDB的偏差。

Method: 提出DeSAE，通过从故意损坏的主链几何中重构自然构象，学习更稳健的结构流形。

Result: DeSAE显著提升了逆折叠任务在多个基准测试中的性能。

Conclusion: 研究揭示了预测结构中的系统性偏差对任务性能的影响，并提出了一种有效的去偏框架，提升了结构学习任务的性能。
...
### [174] [Reinforce LLM Reasoning through Multi-Agent Reflection](https://arxiv.org/abs/2506.08379)
*Yurun Yuan,Tengyang Xie*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在反馈空间和协同训练方面存在不足，限制了性能提升。

Method: 将多轮优化建模为马尔可夫决策过程，引入DPSDP算法，通过直接偏好学习训练actor-critic LLM系统。

Result: 在MATH 500基准上，五轮优化将准确率从58.2%提升至63.2%，验证了多智能体协作和泛化能力。

Conclusion: DPSDP算法有效提升了LLM的推理能力，尤其在多轮优化和泛化任务中表现优异。
...
### [175] [Network Threat Detection: Addressing Class Imbalanced Data with Deep Forest](https://arxiv.org/abs/2506.08383)
*Jiaqi Chen,Rongbin Ye*

Main category: cs.LG

TL;DR: 论文提出了一种基于机器学习的恶意流量检测方法，针对IoT网络中的类不平衡问题，通过重采样策略和集成方法（如gcForest）提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 随着IoT网络的快速扩展，实时检测恶意流量成为关键挑战，研究旨在解决这一问题。

Method: 使用IoT-23数据集，采用三种重采样策略处理类不平衡，并比较多种机器学习技术。

Result: 结合重采样和集成方法（如gcForest）的检测性能优于传统方法。
...
### [176] [Reinforcement Learning Teachers of Test Time Scaling](https://arxiv.org/abs/2506.08388)
*Edoardo Cetin,Tianyu Zhao,Yujin Tang*

Main category: cs.LG

TL;DR: 论文提出了一种新的框架Reinforcement-Learned Teachers (RLTs)，通过避免强化学习的探索挑战，专注于生成最有效的下游蒸馏结果。RLTs通过提供详细解释来训练学生模型，其性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习训练推理语言模型依赖于初始探索能力，且模型主要用于蒸馏新学生而非直接部署。因此，需要一种更高效的框架来优化蒸馏效果。

Method: 引入RLTs框架，RLTs被提示问题和解决方案，任务是生成详细解释以训练学生模型。训练时使用密集奖励，通过学生模型对解释的理解来评估效果。

Result: 7B规模的RLT在竞赛和研究生级任务中表现优于现有蒸馏方法，且能有效训练更大的学生模型和零样本适应新任务。

Conclusion: RLTs框架显著提升了强化学习推理的效率和多用途性，为模型蒸馏和冷启动提供了新思路。
...
### [177] [Spatiotemporal deep learning models for detection of rapid intensification in cyclones](https://arxiv.org/abs/2506.08397)
*Vamshika Sutar,Amandeep Singh,Rohitash Chandra*
...
Method: 使用深度学习模型生成空间坐标和风速数据以复制气旋，并结合数据增强框架进行分类。

Result: 数据增强提高了快速增强事件的检测效果，空间坐标作为输入特征至关重要。
...
### [178] [FUSE: Measure-Theoretic Compact Fuzzy Set Representation for Taxonomy Expansion](https://arxiv.org/abs/2506.08409)
*Fred Xu,Song Jiang,Zijie Huang,Xiao Luo,Shichang Zhang,Adrian Chen,Yizhou Sun*

Main category: cs.LG

TL;DR: 论文提出了一种基于模糊集的集合表示学习方法FUSE，用于分类扩展任务，相比现有方法提升显著。
...
Method: 提出FUSE框架，通过体积近似模糊集来表示集合，满足所有集合操作，且学习效率高，仅需最小神经网络架构。

Result: 在分类扩展任务中，FUSE相比基线方法提升高达23%。

Conclusion: FUSE是首个尝试理解并高效计算模糊集嵌入的工作，为集合表示学习提供了新思路。
...
### [179] [Learning to Hear Broken Motors: Signature-Guided Data Augmentation for Induction-Motor Diagnostics](https://arxiv.org/abs/2506.08412)
*Saraa Ali,Aleksandr Khizhik,Stepan Svirin,Artem Ryzhikov,Denis Derkach*
...
Method: 提出SGDA框架，通过无监督方式在健康电流信号的频域中合成物理合理的故障，结合监督ML和无监督签名分析。

Result: 该方法实现了更高的诊断准确性和可靠性，具有广泛的工业应用潜力。

Conclusion: SGDA为电机诊断领域提供了高效、稳健的解决方案，具有重要的实际应用价值。
...
### [180] [Improved Scaling Laws in Linear Regression via Data Reuse](https://arxiv.org/abs/2506.08415)
*Licong Lin,Jingfeng Wu,Peter L. Bartlett*

Main category: cs.LG

TL;DR: 论文探讨了数据重用如何改进线性回归中的缩放定律，通过多轮随机梯度下降（SGD）在数据受限情况下实现更优的测试误差。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于大规模语言模型训练中数据不足时，如何通过数据重用优化现有缩放定律。

Method: 方法包括在多轮SGD中重用数据，并假设数据协方差和真实参数遵循特定的幂律谱。

Result: 结果表明，多轮SGD在数据受限情况下优于单轮SGD，测试误差更低。

Conclusion: 结论支持数据重用可以改进缩放定律，尤其在数据受限时效果显著。
...
### [181] [Offline RL with Smooth OOD Generalization in Convex Hull and its Neighborhood](https://arxiv.org/abs/2506.08417)
*Qingmao Yao,Zhichao Lei,Tianyuan Chen,Ziyue Yuan,Xuefan Chen,Jianxiang Liu,Faguo Wu,Xiao Zhang*

Main category: cs.LG

TL;DR: 论文提出了一种新方法SQOG，通过平滑贝尔曼算子（SBO）在凸包及其邻域（CHN）内增强Q函数泛化能力，解决了离线强化学习中Q值高估问题，并在D4RL基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习中，分布偏移导致对分布外（OOD）动作的Q值高估，现有方法因过度约束限制了Q函数泛化，影响了策略改进。

Method: 提出平滑贝尔曼算子（SBO），通过将OOD Q值与邻近样本内Q值平滑更新，增强CHN内的Q函数泛化。理论证明SBO能近似真实Q值。

Result: SQOG算法在D4RL基准测试中优于现有方法，实现了接近准确的Q值估计，且在计算效率上表现突出。

Conclusion: SQOG通过SBO解决了过度约束问题，显著提升了离线强化学习的Q值估计和策略性能。
...
### [182] [Online Learning-guided Learning Rate Adaptation via Gradient Alignment](https://arxiv.org/abs/2506.08419)
*Ruichen Jiang,Ali Kavis,Aryan Mokhtari*

Main category: cs.LG

TL;DR: GALA框架通过动态调整学习率，基于梯度对齐和局部曲率估计，无需繁琐的超参数调优，提升优化器在大规模深度学习模型中的性能。
...
Conclusion: GALA为学习率动态调整提供了高效且自适应的解决方案，显著减少了超参数调优的需求。
...
### [183] [HASFL: Heterogeneity-aware Split Federated Learning over Edge Computing Systems](https://arxiv.org/abs/2506.08426)
*Zheng Lin,Zhe Chen,Xianhao Chen,Wei Ni,Yue Gao*

Main category: cs.LG

TL;DR: HASFL框架通过自适应控制批次大小和模型分割，解决了异构边缘设备在分割联邦学习中的性能问题。


<details>
  <summary>Details</summary>
Motivation: 现有分割联邦学习方法因边缘设备能力异构导致性能下降，亟需解决方案。

Method: 提出HASFL框架，通过理论推导收敛边界，自适应调整批次大小和模型分割以优化性能。

Result: 实验验证HASFL在多种数据集上优于现有方法。

Conclusion: HASFL有效平衡了通信计算延迟与训练收敛，适用于异构边缘网络。
...
### [184] [Boosting Gradient Leakage Attacks: Data Reconstruction in Realistic FL Settings](https://arxiv.org/abs/2506.08435)
*Mingyuan Fan,Fuyi Wang,Cen Chen,Jianying Zhou*

Main category: cs.LG

TL;DR: 本文通过实证研究证明，即使在现实的联邦学习环境中，梯度泄漏攻击（GLAs）仍能有效重构客户端数据，并提出FedLeak方法解决性能瓶颈。


<details>
  <summary>Details</summary>
Motivation: 探讨联邦学习（FL）中梯度泄漏攻击的实际风险，填补现有研究中关于攻击有效性的关键空白。

Method: 提出FedLeak方法，包含部分梯度匹配和梯度正则化两种新技术，并设计实际评估协议。
...
### [185] [Learning to Lead: Incentivizing Strategic Agents in the Dark](https://arxiv.org/abs/2506.08438)
*Yuchen Wu,Xinyi Zhong,Zhuoran Yang*
...
### [186] [Time-Aware World Model for Adaptive Prediction and Control](https://arxiv.org/abs/2506.08441)
*Anh N. Nhu,Sanghyun Son,Ming Lin*
...
<details>
  <summary>Details</summary>
Motivation: 传统模型固定时间步长限制了动态学习的灵活性，TAWM旨在解决这一问题。

Method: TAWM通过条件化时间步长Δ𝑡，并在多样化的Δ𝑡值上训练，学习高低频任务动态。

Result: TAWM在多种控制任务中表现优于传统模型，且数据效率更高。

Conclusion: 时间感知的建模方法显著提升了模型性能和数据效率。
...
### [187] [MOBODY: Model Based Off-Dynamics Offline Reinforcement Learning](https://arxiv.org/abs/2506.08460)
*Yihong Guo,Yu Yang,Pan Xu,Anqi Liu*
...
Conclusion: MOBODY通过模型生成和潜在表示学习，有效解决了动态不匹配问题，提升了离线强化学习的性能。
...
### [188] [How to Provably Improve Return Conditioned Supervised Learning?](https://arxiv.org/abs/2506.08463)
*Zhishuai Liu,Yu Yang,Ruhan Wang,Pan Xu,Dongruo Zhou*
...
<details>
  <summary>Details</summary>
Motivation: RCSL虽然简单稳定，但受限于离线数据集质量，缺乏stitching能力。

Method: 提出Reinforced RCSL框架，利用in-distribution optimal return-to-go机制优化策略。

Result: 理论和实验均表明，Reinforced RCSL优于标准RCSL。

Conclusion: Reinforced RCSL是一种有效且简单的改进方法，适用于顺序决策问题。
...
### [189] [MAC: An Efficient Gradient Preconditioning using Mean Activation Approximated Curvature](https://arxiv.org/abs/2506.08464)
*Hyunseok Seung,Jaewoo Lee,Hyunsuk Ko*

Main category: cs.LG

TL;DR: 论文提出了一种名为MAC的高效二阶优化方法，通过近似Kronecker因子降低计算负担，优于KFAC和其他现有方法。


<details>
  <summary>Details</summary>
Motivation: 二阶优化方法（如KFAC）利用损失曲面的曲率信息提升收敛性，但计算成本高。

Method: 分析Kronecker因子（激活和预激活梯度），提出高效近似方法，应用于注意力层的FIM。

Result: MAC在准确性、训练时间和内存使用上优于KFAC和其他方法。

Conclusion: MAC是首个将Kronecker分解应用于Transformer注意力层的方法，并在实验中表现优异。
...
### [190] [AsFT: Anchoring Safety During LLM Fine-Tuning Within Narrow Safety Basin](https://arxiv.org/abs/2506.08473)
*Shuo Yang,Qihui Zhang,Yuyang Liu,Yue Huang,Xiaojun Jia,Kunpeng Ning,Jiayu Yao,Jigang Wang,Hailiang Dai,Yibing Song,Li Yuan*
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在微调过程中容易受到安全风险的影响，少量恶意或无害数据可能破坏模型的安全性。

Method: 基于对齐方向的概念，提出AsFT方法，通过正则化项约束微调过程，确保其在安全范围内进行。

Result: 实验表明，AsFT在多个数据集上优于Safe LoRA，减少有害行为7.60%，提升性能3.44%。

Conclusion: AsFT通过锚定对齐方向，有效约束微调过程，显著提升模型的安全性和性能。
...
### [191] [Thermodynamically Consistent Latent Dynamics Identification for Parametric Systems](https://arxiv.org/abs/2506.08475)
*Xiaolong He,Yeonjong Shin,Anthony Gruber,Sohyeon Jung,Kookjin Lee,Youngsoo Choi*

Main category: cs.LG

TL;DR: 提出了一种基于热力学原理的潜在空间动力学识别框架（tLaSDI），用于参数非线性动力系统的降阶建模，结合自编码器和参数化GENERIC神经网络，实现了高效学习和物理一致性。


<details>
  <summary>Details</summary>
Motivation: 解决参数非线性动力系统的高效建模问题，同时保持热力学原理（如自由能守恒和熵生成）。

Method: 结合自编码器降维和参数化GENERIC神经网络（pGFINNs），并采用基于残差的主动学习策略优化训练数据采样。

Result: 在Burgers方程和1D/1V Vlasov-Poisson方程上实现了最高3,528倍加速，相对误差1-3%，训练和推理成本显著降低。

Conclusion: tLaSDI框架不仅高效且物理一致，还能揭示系统的热力学行为，为物理空间动力学提供新见解。
...
### [192] [Explaining, Fast and Slow: Abstraction and Refinement of Provable Explanations](https://arxiv.org/abs/2506.08505)
*Shahaf Bassan,Yizhak Yisrael Elboher,Tobias Ladner,Matthias Althoff,Guy Katz*

Main category: cs.LG

TL;DR: 提出了一种新颖的抽象-精化技术，用于高效计算神经网络预测的可证明充分解释，解决了现有方法在可扩展性上的挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管事后解释技术有所进展，但许多方法依赖启发式且缺乏形式化保证。现有可证明解释方法计算效率低，难以扩展。

Method: 通过构建一个显著简化的网络抽象原始大网络，在简化网络上计算充分解释，若不足则逐步精化网络直至收敛。

Result: 实验表明，该方法显著提高了获取可证明充分解释的效率，并提供了网络预测在不同抽象层次上的细粒度解释。

Conclusion: 该方法为神经网络预测提供了高效且可证明的解释框架，同时支持多层次的解释分析。
...
### [193] [DiffGradCAM: A Universal Class Activation Map Resistant to Adversarial Training](https://arxiv.org/abs/2506.08514)
*Jacob Piland,Chris Sweet,Adam Czakja*

Main category: cs.LG

TL;DR: 论文提出了SHAM和DiffGradCAM，用于评估和改进CAM方法的鲁棒性，解决了CAM在对抗条件下的脆弱性问题。
...
Method: 提出了SHAM作为对抗条件下的基准测试工具，并开发了DiffGradCAM，一种轻量级且对比性的CAM方法，能够抵抗被动欺骗。

Result: SHAM和DiffGradCAM在多分类任务中验证了其有效性，DiffGradCAM在非对抗情况下与标准CAM方法表现一致。
...
### [194] [NeurIPS 2024 ML4CFD Competition: Results and Retrospective Analysis](https://arxiv.org/abs/2506.08516)
*Mouadh Yagoubi,David Danan,Milad Leyli-Abadi,Ahmed Mazari,Jean-Patrick Brunet,Abbas Kabalan,Fabien Casenave,Yuxin Ma,Giovanni Catalani,Jean Fesquet,Jacob Helwig,Xuan Zhang,Haiyang Yu,Xavier Bertrand,Frederic Tost,Michael Baurheim,Joseph Morlier,Shuiwang Ji*

Main category: cs.LG

TL;DR: ML4CFD竞赛通过评估机器学习在CFD中的表现，展示了ML模型在特定条件下超越传统求解器的潜力。


<details>
  <summary>Details</summary>
Motivation: 解决机器学习在物理科学中应用的准确性、泛化性和物理一致性挑战。

Method: 组织ML4CFD竞赛，提供OpenFOAM生成的数据集，评估预测准确性、物理保真度等多项标准。

Result: 顶级参赛作品在综合指标上超越OpenFOAM求解器。

Conclusion: ML模型在科学计算中具有潜力，未来需优化评估框架和设计原则。
...
### [195] [Leveraging chaos in the training of artificial neural networks](https://arxiv.org/abs/2506.08523)
*Pedro Jiménez-González,Miguel C. Soriano,Lucas Lacasa*

Main category: cs.LG

TL;DR: 论文研究了在异常大学习率下梯度下降（GD）优化神经网络的动态行为，发现学习率在特定范围内时，GD会从纯利用型转变为探索-利用平衡型，且训练时间最短。


<details>
  <summary>Details</summary>
Motivation: 探索神经网络在非传统大学习率下的训练动态，揭示混沌动力学在神经网络训练中的积极作用。

Method: 通过分析神经网络轨迹的最大Lyapunov指数，研究GD在不同学习率下的行为，并在MNIST分类任务中验证。

Result: 在学习率特定范围内，GD表现出探索-利用平衡，训练时间最短，且结果适用于多种学习任务和架构。

Conclusion: 研究表明，混沌动力学在神经网络训练中具有建设性作用，可通过调整学习率优化训练效率。
...
### [196] [Robust Evolutionary Multi-Objective Network Architecture Search for Reinforcement Learning (EMNAS-RL)](https://arxiv.org/abs/2506.08533)
*Nihal Acharya Adde,Alexandra Gianzina,Hanno Gottschalk,Andreas Ebert*

Main category: cs.LG

TL;DR: EMNAS首次提出用于大规模强化学习中的神经网络架构优化，通过遗传算法自动设计网络，提升奖励并减少模型大小，同时采用并行化和师生方法加速搜索与优化。实验表明其优于人工设计模型。


<details>
  <summary>Details</summary>
Motivation: 优化自动驾驶中的强化学习网络架构，提升性能并减少模型复杂度，同时探索迁移学习在迭代学习中的潜力。

Method: 使用遗传算法自动化网络设计，结合并行化技术和师生方法加速搜索与优化。

Result: EMNAS在奖励和参数效率上优于人工设计模型，验证了其在自动驾驶中的有效性。

Conclusion: EMNAS为自动驾驶强化学习提供了高效网络设计框架，推动领域向更优性能发展。
...
### [197] [DeepForm: Reasoning Large Language Model for Communication System Formulation](https://arxiv.org/abs/2506.08551)
*Panlong Wu,Ting Wang,Yifei Zhong,Haoqi Zhang,Zitong Wang,Fangxin Wang*

Main category: cs.LG

TL;DR: DeepForm是一个专为通信系统设计优化的推理型大语言模型，通过两阶段训练策略和高质量数据集CSFRC，显著优于现有通用模型。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在通信系统设计领域缺乏专业知识和高质量数据，DeepForm旨在填补这一空白。

Method: 采用两阶段训练：监督微调（SFT）结合链式思维（CoT）数据，以及基于ReMax的规则强化学习算法C-ReMax。

Result: 实验表明DeepForm在多样化场景中表现优异，超越现有大型专有模型。

Conclusion: DeepForm为通信系统设计提供了高效解决方案，相关资源将开源以推动进一步研究。
...
### [198] [The Geometries of Truth Are Orthogonal Across Tasks](https://arxiv.org/abs/2506.08572)
*Waiss Azizian,Michael Kirchhof,Eugene Ndiaye,Louis Bethune,Michal Klein,Pierre Ablin,Marco Cuturi*
...
<details>
  <summary>Details</summary>
Motivation: 探讨LLMs激活模式在评估答案正确性时的局限性，尤其是跨任务的可迁移性。

Method: 通过线性分类器分析不同任务中LLMs的激活模式，并尝试使用稀疏正则化和复杂方法（如混合探针）改进。

Result: 发现不同任务的线性分类器相似性低，激活向量在跨任务时形成明显分离的簇。

Conclusion: LLMs的“真理几何”具有任务依赖性，现有方法难以实现跨任务迁移。
...
### [199] [SLEEPYLAND: trust begins with fair evaluation of automatic sleep staging models](https://arxiv.org/abs/2506.08574)
*Alvise Dei Rossi,Matteo Metaldi,Michal Bechny,Irina Filchenko,Julia van der Meer,Markus H. Schmidt,Claudio L. A. Bassetti,Athina Tzovara,Francesca D. Faraci,Luigi Fiorillo*

Main category: cs.LG

TL;DR: SLEEPYLAND是一个开源睡眠分期评估框架，旨在解决模型评估、泛化、偏见和标注变异性问题，包含大量ID和OOD数据，并提出了SOMNUS集成模型，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 临床采用深度学习进行自动睡眠分期的进展受限，主要由于模型评估不公平、泛化能力差、模型偏见和标注变异性等问题。

Method: 提出SLEEPYLAND框架，包含大量ID和OOD数据，并开发SOMNUS集成模型，通过软投票结合不同架构和通道配置的模型。

Result: SOMNUS在24个数据集上表现稳健，优于现有方法和人类评分者，同时量化了模型偏见并提出了预测标注分歧的指标。

Conclusion: SLEEPYLAND和SOMNUS显著提升了睡眠分期的性能和鲁棒性，但仍需进一步减少模型偏见。
...
### [200] [Diffusion-based Time Series Forecasting for Sewerage Systems](https://arxiv.org/abs/2506.08577)
*Nicholas A. Pearson,Francesca Cairoli,Luca Bortolussi,Davide Russo,Francesca Zanello*
...
<details>
  <summary>Details</summary>
Motivation: 提升污水系统在极端天气条件下的预测准确性，捕捉复杂环境信号间的关联。

Method: 开发了一种基于扩散的模型处理多元时间序列数据，并结合保形推理技术校准预测区间。

Result: 模型在真实污水系统数据上表现出色，即使在极端天气下也能保持高准确性。

Conclusion: 该方法为污水系统的上下文预测提供了可靠且统计有效的解决方案。
...
### [201] [CALT: A Library for Computer Algebra with Transformer](https://arxiv.org/abs/2506.08600)
*Hiroshi Kera,Shun Arakawa,Yuta Sato*

Main category: cs.LG

TL;DR: 论文介绍了CALT，一个用户友好的Python库，帮助非深度学习专家训练模型进行符号计算任务。


<details>
  <summary>Details</summary>
Motivation: 近年来AI的进步表明符号计算可以通过端到端深度学习学习，但需要符号计算社区的积极参与。

Method: 使用Transformer模型训练，通过大量符号表达式的示例模拟计算。

Result: 开发了CALT库，为非专家提供便捷的工具。

Conclusion: CALT为符号计算任务提供了新的研究方向和实践工具。
...
### [202] [Flow Matching Meets PDEs: A Unified Framework for Physics-Constrained Generation](https://arxiv.org/abs/2506.08604)
*Giacomo Baldan,Qiang Liu,Alberto Guardone,Nils Thuerey*

Main category: cs.LG

TL;DR: 论文提出了一种名为PBFM的新生成框架，通过显式嵌入物理约束（PDE残差和代数关系）到流匹配目标中，提高了物理模拟的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统生成方法（如扩散模型和流匹配）通常从数据中隐式学习物理规律，缺乏显式物理约束，限制了其准确性和效率。

Method: PBFM结合流匹配损失和基于物理的残差损失，无需调整超参数权重，并引入时间展开训练以提高预测精度。

Result: 在三个代表性PDE问题上，PBFM的物理残差比传统方法准确8倍，且在分布准确性上优于现有算法。
...
### [203] [Sample Efficient Demonstration Selection for In-Context Learning](https://arxiv.org/abs/2506.08607)
*Kiran Purohit,V Venktesh,Sourangshu Bhattacharya,Avishek Anand*

Main category: cs.LG

TL;DR: 论文提出了一种名为CASE的高效样本选择策略，用于在上下文学习中选择最优的few-shot示例，显著减少了LLM评估次数和运行时间。


<details>
  <summary>Details</summary>
Motivation: 在上下文学习中，few-shot示例的选择对构建高效提示至关重要，但现有方法在样本复杂性和计算成本上存在挑战。

Method: 将示例选择问题建模为top-m最佳臂识别问题，提出CASE策略，通过维护“挑战者”臂短列表和线性评分函数，减少样本复杂性和LLM调用次数。

Result: CASE实现了高达7倍的运行时间加速和87%的LLM调用减少，同时保持性能。
...
### [204] [HSG-12M: A Large-Scale Spatial Multigraph Dataset](https://arxiv.org/abs/2506.08618)
*Xianquan Yan,Hakan Akgün,Kenji Kawaguchi,N. Duane Loh,Ching Hua Lee*
...
Method: 通过Poly2Graph管道，将一维晶体哈密顿量映射为谱图，生成多样化的物理基础拓扑结构。

Result: HSG-12M包含1160万静态和510万动态谱图，覆盖1401个特征多项式类。GNN基准测试揭示了多边几何学习的新挑战。

Conclusion: HSG-12M为几何感知图学习奠定了基础，并开辟了数据驱动科学发现的新机会。
...
### [205] [Time Series Representations for Classification Lie Hidden in Pretrained Vision Transformers](https://arxiv.org/abs/2506.08641)
*Simon Roschmann,Quentin Bouniot,Vasilii Feofanov,Ievgen Redko,Zeynep Akata*

Main category: cs.LG

TL;DR: 提出了一种将时间序列转换为图像以利用预训练视觉Transformer（ViT）的框架TiViT，在时间序列分类任务中取得先进性能。
...
Method: 将时间序列转换为图像，利用预训练的ViT模型（如OpenCLIP）提取隐藏表示，分析其2D分块对时间序列的影响。

Result: TiViT在标准时间序列分类基准上达到最先进性能，发现高内在维度的中间层最有效。
...
### [206] [Semi-gradient DICE for Offline Constrained Reinforcement Learning](https://arxiv.org/abs/2506.08644)
*Woosung Kim,JunHo Seo,Jongmin Lee,Byung-Jun Lee*

Main category: cs.LG

TL;DR: DICE方法用于解决策略诱导的稳态分布与目标分布不匹配问题，但现有改进方法削弱了其离线策略评估（OPE）能力。本文提出新方法，通过半梯度DICE实现OPE和约束RL。


<details>
  <summary>Details</summary>
Motivation: 解决DICE框架在离线RL中因半梯度优化导致的OPE能力下降问题，提升约束RL性能。

Method: 提出一种新方法，通过半梯度DICE优化，确保准确的成本估计。

Result: 在离线约束RL基准DSRL上达到最优性能。

Conclusion: 新方法成功解决了半梯度DICE的局限性，同时提升了OPE和约束RL的效果。
...
### [207] [Fusing Cross-modal and Uni-modal Representations: A Kronecker Product Approach](https://arxiv.org/abs/2506.08645)
*Youqi Wu,Jingwei Zhang,Farzan Farnia*

Main category: cs.LG

TL;DR: RP-KrossFuse方法通过随机投影和Kronecker积整合跨模态和单模态嵌入，在保持跨模态对齐的同时提升单模态任务的性能。


<details>
  <summary>Details</summary>
Motivation: 解决跨模态嵌入在单模态任务上表现不佳，而单模态嵌入缺乏跨模态对齐能力的问题。

Method: 提出RP-KrossFuse方法，利用随机投影和Kronecker积融合跨模态和单模态嵌入的相似性得分，支持高效实现。

Result: 实验表明，RP-KrossFuse在单模态任务上表现优异，同时保持跨模态对齐能力。
...
### [208] [JoFormer (Journey-based Transformer): Theory and Empirical Analysis on the Tiny Shakespeare Dataset](https://arxiv.org/abs/2506.08652)
*Mahesh Godavarti*

Main category: cs.LG

TL;DR: JoFormer是一种基于非交换代数的Transformer架构，通过可学习的定向变换表示相对位置，在语言建模任务中表现优于RoFormer，具有更低的困惑度和更快的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在序列建模中表现出色，但如何有效融入位置信息仍是一个挑战。JoFormer旨在通过非交换代数提供一种更灵活的位置表示方法。

Method: JoFormer采用基于旅程的架构，通过可学习的定向变换序列化组合来表示相对位置，并推导出新的注意力机制。

Result: 在Tiny Shakespeare任务中，JoFormer比RoFormer表现更好，困惑度更低且收敛更快。

Conclusion: JoFormer为Transformer架构提供了一种基于原则的位置信息集成方法，展示了其作为更复杂架构原型的潜力。
...
### [209] [When Simple Model Just Works: Is Network Traffic Classification in Crisis?](https://arxiv.org/abs/2506.08655)
*Kamil Jerabek,Jan Luxemburk,Richard Plny,Josef Koumar,Jaroslav Pesek,Karel Hynek*
...
<details>
  <summary>Details</summary>
Motivation: 研究为何简单的k-NN方法在网络流量分类中能与复杂神经网络媲美甚至更优。

Method: 评估k-NN基线在12个数据集和15个分类任务中的表现，分析数据冗余对性能的影响。

Result: 数据集中超过50%的冗余样本导致模型性能被高估，且标准机器学习方法可能不适用于流量分类。

Conclusion: 提出新的任务设计和评估方向，以解决数据冗余问题并推动领域发展。
...
### [210] [Towards Robust Real-World Multivariate Time Series Forecasting: A Unified Framework for Dependency, Asynchrony, and Missingness](https://arxiv.org/abs/2506.08660)
*Jinkwan Jang,Hyungjin Park,Jinmyeong Choi,Taesup Kim*
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的时间序列数据通常是多变量的，且存在复杂的通道间依赖关系、异步采样和缺失值问题，而现有模型通常基于过于简化的假设，无法应对这些挑战。

Method: 提出ChannelTokenFormer，通过灵活的架构显式捕捉跨通道交互，适应通道异步采样，并有效处理缺失值。

Result: 在三个基准数据集和一个真实工业数据集上的实验表明，ChannelTokenFormer在复杂现实条件下具有优越的鲁棒性和准确性。

Conclusion: ChannelTokenFormer为解决现实世界时间序列预测中的关键挑战提供了一种有效方法。
...
### [211] [Optimizing Learned Image Compression on Scalar and Entropy-Constraint Quantization](https://arxiv.org/abs/2506.08662)
*Florian Borzechowski,Michael Schäfer,Heiko Schwarz,Jonathan Pfaff,Detlev Marpe,Thomas Wiegand*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在训练过程中无法准确模拟量化噪声，导致网络性能次优。

Method: 在传统端到端训练后，对网络部分进行基于量化潜在表示的重新训练。

Result: 实验表明，该方法在均匀标量量化和熵约束量化下均能带来编码增益，最高可达2.2%的比特率节省。

Conclusion: 通过正确量化数据的重新训练，可以显著提升图像压缩的性能，且不增加推理复杂度。
...
### [212] [Enhancing Reasoning Capabilities of Small Language Models with Blueprints and Prompt Template Search](https://arxiv.org/abs/2506.08669)
*Dongge Han,Menglin Xia,Daniel Madrigal Diaz,Samuel Kessler,Ankur Mallick,Xuchao Zhang,Mirian Del Carmen Hipolito Garcia,Jin Xu,Victor Rühle,Saravan Rajmohan*

Main category: cs.LG

TL;DR: 提出了一种通过LLM生成的蓝图增强小型语言模型（SLM）推理能力的框架，并解决了SLM对提示变化的敏感性问题。


<details>
  <summary>Details</summary>
Motivation: SLM虽然高效，但其有限的推理能力和对提示变化的敏感性限制了其应用。

Method: 利用LLM生成结构化推理蓝图，并结合提示模板搜索机制。

Result: 在数学、编程和逻辑推理任务中显著提升了SLM的性能。
...
### [213] [Towards Fair Representation: Clustering and Consensus](https://arxiv.org/abs/2506.08673)
*Diptarka Chakraborty,Kushagra Chatterjee,Debarati Das,Tien Long Nguyen,Romina Nobahari*

Main category: cs.LG

TL;DR: 该论文研究了共识聚类中的公平性问题，首次提出了一种常数因子近似算法，并探讨了如何最小化修改现有聚类以实现公平性。


<details>
  <summary>Details</summary>
Motivation: 共识聚类通常基于非敏感属性，但缺乏对公平性的考虑。该研究旨在将公平性引入共识聚类，确保每个受保护组在聚类中得到比例代表。

Method: 研究开发了针对等比例组的最优算法，以及针对不等比例组的近线性时间常数因子近似算法，并证明了问题的NP难度。

Result: 提出了首个公平共识聚类的常数因子近似算法，并展示了其在不等比例组中的高效性。
...
### [214] [Mitigating Reward Over-optimization in Direct Alignment Algorithms with Importance Sampling](https://arxiv.org/abs/2506.08681)
*Phuc Minh Nguyen,Ngoc-Hieu Nguyen,Duy H. M. Nguyen,Anji Liu,An Mai,Binh T. Nguyen,Daniel Sonntag,Khoa D. Doan*

Main category: cs.LG

TL;DR: 本文提出了一种基于重要性采样的方法（IS-DAAs），用于缓解直接对齐算法（DAAs）中的过优化问题，并通过实验验证其有效性。
...
Method: 提出了一种重要性采样方法（IS-DAAs），通过乘上一个重要性比率来考虑参考策略分布，并通过裁剪比率来避免高方差问题。
...
### [215] [Variational Autoencoder-Based Approach to Latent Feature Analysis on Efficient Representation of Power Load Monitoring Data](https://arxiv.org/abs/2506.08698)
*Boyu Xie,Tangtang Xie*
...
<details>
  <summary>Details</summary>
Motivation: 智能电网中高维不完整的PLM数据对PLF模型性能提出挑战，需要高效的数据表示和补全方法。
...
### [216] [Breaking the ICE: Exploring promises and challenges of benchmarks for Inference Carbon & Energy estimation for LLMs](https://arxiv.org/abs/2506.08727)
*Samarth Sikand,Rohit Mehra,Priyavanshi Pathania,Nikhil Bamby,Vibhu Saujanya Sharma,Vikrant Kaulgud,Sanjay Podder,Adam P. Burden*

Main category: cs.LG

TL;DR: 论文探讨了生成式AI（如LLMs）对能源和环境的影响，提出了一种基于现有SOTA基准的框架R-ICE，用于估算推理碳排放，解决了现有工具的不足。
...
Method: 提出R-ICE框架，利用现有SOTA基准数据估算推理碳排放，克服了高数据需求、侵入性和误差大的问题。

Result: 验证结果表明，基于基准的建模在估算碳排放方面具有潜力，支持动态LLM路由和碳核算等应用。
...
### [217] [Exploration by Random Reward Perturbation](https://arxiv.org/abs/2506.08737)
*Haozhe Ma,Guoji Fu,Zhengding Luo,Jiele Wu,Tze-Yun Leong*

Main category: cs.LG

TL;DR: RRP是一种新的强化学习探索策略，通过向环境奖励添加零均值噪声增强策略多样性，提升探索范围。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习中探索不足的问题，通过奖励扰动增强策略多样性。

Method: 在环境奖励中添加零均值噪声，与现有探索策略兼容。

Result: 显著提升PPO和SAC的性能，提高样本效率并避免局部最优。

Conclusion: RRP是一种轻量级、通用的探索策略，能有效提升强化学习算法的性能。
...
### [218] [Urban Incident Prediction with Graph Neural Networks: Integrating Government Ratings and Crowdsourced Reports](https://arxiv.org/abs/2506.08740)
*Sidhika Balachandar,Shuvom Sadhuka,Bonnie Berger,Emma Pierson,Nikhil Garg*

Main category: cs.LG

TL;DR: 提出了一种基于GNN的多视图多输出模型，结合政府检查数据和众包报告数据预测城市事件的真实状态，并在纽约市案例中验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 政府检查数据稀疏且众包报告数据存在偏差，需要一种方法结合两者预测事件的真实状态。

Method: 使用多视图、多输出的GNN模型，整合政府检查数据和众包报告数据。

Result: 模型在真实和半合成数据上表现优于仅使用单一数据源的模型，尤其是在检查数据稀疏时。同时量化了众包报告的偏差。
...
### [219] [On the Stability of the Jacobian Matrix in Deep Neural Networks](https://arxiv.org/abs/2506.08764)
*Benjamin Dadoun,Soufiane Hayou,Hanan Salam,Mohamed El Amine Seddik,Pierre Youssef*

Main category: cs.LG

TL;DR: 本文提出了一种适用于稀疏性和非独立同分布权重的深度神经网络稳定性定理，扩展了现代神经网络初始化方案的理论基础。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在深度增加时容易出现梯度爆炸或消失问题，现有研究多限于全连接网络和独立同分布权重，本文旨在突破这些限制。

Method: 利用随机矩阵理论的最新进展，分析稀疏性和非独立同分布权重对网络稳定性的影响。

Result: 建立了适用于更广泛网络模型的谱稳定性理论，为结构化权重和依赖随机性的初始化方案提供了严格保证。

Conclusion: 本文扩展了深度神经网络稳定性理论，为现代网络中的初始化方案提供了更广泛的理论支持。
...
### [220] [Design Patterns for Securing LLM Agents against Prompt Injections](https://arxiv.org/abs/2506.08837)
*Luca Beurer-Kellner,Beat Buesser Ana-Maria Creţu,Edoardo Debenedetti,Daniel Dobos,Daniel Fabian,Marc Fischer,David Froelicher,Kathrin Grosse,Daniel Naeff,Ezinwanne Ozoani,Andrew Paverd,Florian Tramèr,Václav Volhejn*

Main category: cs.LG

TL;DR: 论文提出了一套设计模式，用于构建能够抵抗提示注入攻击的AI代理，并分析了这些模式在实用性和安全性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 随着基于大语言模型的AI代理能力增强，其安全性问题日益突出，尤其是提示注入攻击对代理的威胁。

Method: 提出了一套设计模式，系统分析了这些模式，并通过案例研究展示了其实际应用。

Result: 设计模式能够为AI代理提供可证明的抵抗提示注入攻击的能力。

Conclusion: 这些设计模式在实用性和安全性之间取得了平衡，为构建安全的AI代理提供了有效方法。
...
### [221] [IMAGIC-500: IMputation benchmark on A Generative Imaginary Country (500k samples)](https://arxiv.org/abs/2506.08844)
*Siyi Sun,David Antony Selby,Yunchuan Huang,Sebastian Vollmer,Seth Flaxman,Anisoara Calinescu*

Main category: cs.LG

TL;DR: 该论文利用公开的合成数据集IMAGIC-500，为社会经济数据中的缺失值填补方法提供了全面的基准测试，评估了多种填补技术的效果。


<details>
  <summary>Details</summary>
Motivation: 社会经济数据因隐私保护难以公开共享，导致缺失值填补方法的研究缺乏可复现性和基准数据集。

Method: 使用世界银行的合成数据集，构建IMAGIC-500，并在不同缺失机制和比例下测试多种填补方法。

Result: 评估了统计、传统机器学习和深度学习方法的效果，揭示了各种技术的优缺点。

Conclusion: IMAGIC-500数据集和基准测试有助于开发鲁棒的填补算法，促进社会科学研究的可复现性。
...
### [222] [Agile Reinforcement Learning for Real-Time Task Scheduling in Edge Computing](https://arxiv.org/abs/2506.08850)
*Amin Avan,Akramul Azim,Qusay Mahmoud*

Main category: cs.LG

TL;DR: 本文提出了一种敏捷强化学习（aRL）方法，用于边缘计算中软实时应用的任务调度，通过智能探索和动作屏蔽技术提高调度效率和收敛速度。


<details>
  <summary>Details</summary>
Motivation: 边缘计算环境中任务调度的复杂性、动态性和多目标性使得传统启发式和元启发式算法难以生成最优调度方案，而强化学习算法的学习时间过长也限制了其应用。

Method: 提出aRL方法，结合智能探索和动作屏蔽技术，减少无关动作的随机探索，提升RL-agent的预测能力和适应性。

Result: 实验表明，aRL在命中率和收敛速度上优于基线方法。

Conclusion: aRL是一种适合边缘计算中软实时应用任务调度的有效方法。
...
### [223] [Adapting to Heterophilic Graph Data with Structure-Guided Neighbor Discovery](https://arxiv.org/abs/2506.08871)
*Victor M. Tenorio,Madeline Navarro,Samuel Rey,Santiago Segarra,Antonio G. Marques*
...
<details>
  <summary>Details</summary>
Motivation: GNN通常假设同质性，但在异质性数据（连接节点标签不同）上表现不佳，因此需要改进。

Method: 通过连接具有相似结构属性的节点创建新图结构，并引入SG-GNN，自适应地学习原始图和新图的权重。

Result: 在多个异质性数据集上，SG-GNN达到了最先进或极具竞争力的性能。

Conclusion: 利用结构信息引导GNN能有效提升其在异质性数据上的表现。
...
### [224] [Filling in the Blanks: Applying Data Imputation in incomplete Water Metering Data](https://arxiv.org/abs/2506.08882)
*Dimitrios Amaxilatis,Themistoklis Sarantakos,Ioannis Chatzigiannakis,Georgios Mylonas*

Main category: cs.LG

TL;DR: 研究探讨了数据填补技术在水分配网络监测中的应用，比较了多种方法对数据质量和可靠性的影响。


<details>
  <summary>Details</summary>
Motivation: 智能水表数据存在缺失，影响决策效率，需填补技术提升数据质量。

Method: 比较了k近邻、MissForest、Transformer和循环神经网络等填补方法。
...
### [225] [InfoDPCCA: Information-Theoretic Dynamic Probabilistic Canonical Correlation Analysis](https://arxiv.org/abs/2506.08884)
*Shiqin Tang,Shujian Yu*

Main category: cs.LG

TL;DR: InfoDPCCA是一种动态概率CCA框架，用于提取两个相关序列的共享潜在表示，同时学习序列特定的信息。
...
### [226] [SeerAttention-R: Sparse Attention Adaptation for Long Reasoning](https://arxiv.org/abs/2506.08889)
*Yizhao Gao,Shuming Guo,Shijie Cao,Yuqing Xia,Yu Cheng,Lei Wang,Lingxiao Ma,Yutao Sun,Tianzhu Ye,Li Dong,Hayden Kwok-Hay So,Yu Hua,Ting Cao,Fan Yang,Mao Yang*
...
<details>
  <summary>Details</summary>
Motivation: 解决长解码任务中稀疏注意力机制的效率和性能问题，同时保持推理准确性。

Method: 扩展SeerAttention框架，移除查询池化以适应自回归解码，并引入轻量级门控机制。

Result: 在AIME基准测试中，仅用0.4B token训练，SeerAttention-R在4K token预算下保持接近无损的推理准确性，并在H100 GPU上实现9倍速度提升。

Conclusion: SeerAttention-R是一种高效且灵活的稀疏注意力框架，适用于长解码任务，且易于集成到现有预训练模型中。
...
### [227] [Intention-Conditioned Flow Occupancy Models](https://arxiv.org/abs/2506.08902)
*Chongyi Zheng,Seohong Park,Sergey Levine,Benjamin Eysenbach*

Main category: cs.LG

TL;DR: 论文提出了一种基于意图条件流占用模型（InFOM）的强化学习预训练方法，通过预测未来状态分布和用户意图，显著提升了任务表现。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习中预训练模型的挑战，如动作的长期依赖性和样本效率，利用生成式AI的新工具建模复杂分布。

Method: 使用流匹配构建概率模型预测未来状态分布，引入用户意图的隐变量增强模型表达能力，支持广义策略改进。

Result: 在36个状态基准和4个图像基准任务中，InFOM方法的中位数回报提升1.8倍，成功率提高36%。

Conclusion: InFOM通过建模未来状态和用户意图，显著提升了强化学习预训练的效果，为领域提供了新工具。
...
### [228] [Enhancing generalizability of model discovery across parameter space with multi-experiment equation learning (ME-EQL)](https://arxiv.org/abs/2506.08916)
*Maria-Veronica Ciocanel,John T. Nardini,Kevin B. Flores,Erica M. Rutter,Suzanne S. Sindi,Alexandria Volkening*
...
<details>
  <summary>Details</summary>
Motivation: 解决传统方程学习方法（EQL）因需大量模拟而泛化性不足的问题。

Method: 提出OAT ME-EQL（逐个参数学习并插值）和ES ME-EQL（跨参数统一模型库）两种方法。
...
### [229] [Local MDI+: Local Feature Importances for Tree-Based Models](https://arxiv.org/abs/2506.08928)
*Zhongyuan Liang,Zachary T. Rewolinski,Abhineet Agarwal,Tiffany M. Tang,Bin Yu*

Main category: cs.LG

TL;DR: 论文提出了一种名为LMDI+的新方法，用于解决现有局部特征重要性方法（如LIME和TreeSHAP）的不足，通过扩展全局MDI+框架，实现了更高的性能和稳定性。


<details>
  <summary>Details</summary>
Motivation: 在需要可解释性的高风险领域，现有局部特征重要性方法依赖近似且不稳定的扰动，无法充分利用模型内部结构。

Method: 提出Local MDI+（LMDI+），将MDI+框架扩展到样本特定场景，利用决策树与线性模型在节点基础上的等价性。

Result: LMDI+在12个真实基准数据集上平均提升10%的下游任务性能，并表现出更高的稳定性。
...
### [230] [BioLangFusion: Multimodal Fusion of DNA, mRNA, and Protein Language Models](https://arxiv.org/abs/2506.08936)
*Amina Mollaysa,Artem Moskale,Pushpak Pati,Tommaso Mansi,Mangal Prakash,Rui Liao*

Main category: cs.LG

TL;DR: BioLangFusion整合DNA、mRNA和蛋白质预训练语言模型，通过三种融合技术实现跨模态统一表示，无需额外预训练，在分子属性预测任务中表现优于单模态基线。


<details>
  <summary>Details</summary>
Motivation: 基于分子生物学的中心法则（基因到转录到蛋白质的信息流），通过生物意义明确的密码子级别对齐，实现跨模态直接对应。

Method: 采用三种标准融合技术：(i) 密码子级嵌入拼接，(ii) 基于多实例学习的熵正则化注意力池化，(iii) 跨模态多头注意力。

Result: 在五个分子属性预测任务中，BioLangFusion表现优于单模态基线，表明简单融合预训练模型即可高效捕获多组学互补信息。

Conclusion: BioLangFusion展示了预训练模型简单融合的潜力，能以最小开销整合多模态信息。
...
### [231] [KARMA: A Multilevel Decomposition Hybrid Mamba Framework for Multivariate Long-Term Time Series Forecasting](https://arxiv.org/abs/2506.08939)
*Hang Ye,Gaoxiang Duan,Haoran Zeng,Yangxin Zhu,Lingxue Meng,Xiaoying Zheng,Yongxin Zhu*

Main category: cs.LG

TL;DR: KARMA提出了一种自适应时间通道分解模块（ATCD）和混合频时分解模块（HFTD），结合多尺度Mamba-based KarmaBlock，显著提升了时间序列预测的精度和计算效率。
...
Method: KARMA通过ATCD动态提取趋势和季节性成分，HFTD进一步分解序列为频域和时域，结合KarmaBlock高效处理全局和局部信息。

Result: 在八个真实数据集上，KARMA在预测精度和计算效率上均显著优于主流基线方法。

Conclusion: KARMA通过动态分解和高效信息处理，解决了传统方法和Transformer模型的局限性，为时间序列预测提供了有效方案。
...
### [232] [Towards Robust Deep Reinforcement Learning against Environmental State Perturbation](https://arxiv.org/abs/2506.08961)
*Chenxu Wang,Huaping Liu*
...
<details>
  <summary>Details</summary>
Motivation: 现有研究较少关注环境状态扰动，而这类扰动在具体场景中很常见。为提高DRL代理的鲁棒性，论文探讨了这一问题。

Method: 首先提出一种非目标攻击方法作为校准对手，随后提出BAT防御框架，结合监督学习和对抗训练。

Result: 实验验证了主流代理在环境状态扰动下的脆弱性及BAT框架的有效性。
...
### [233] [GFRIEND: Generative Few-shot Reward Inference through EfficieNt DPO](https://arxiv.org/abs/2506.08965)
*Yiyang Zhao,Huiyu Bai,Xuejiao Zhao*

Main category: cs.LG

TL;DR: 提出了一种数据增强和扩展框架，通过偏好细化和多级直接偏好优化（M-DPO），在小数据集上训练生成奖励模型，性能媲美大规模数据集训练的结果。


<details>
  <summary>Details</summary>
Motivation: 提升从人类反馈中强化学习（RLHF）的效率和可扩展性，解决传统方法在样本配对和数据多样性上的不足。

Method: 采用偏好细化（Chain-of-Thought采样）和困惑度评分机制，结合多级直接偏好优化（M-DPO）来捕捉细粒度偏好差异。

Result: 实验表明，该方法显著提高了数据效率和模型性能，小样本训练的奖励模型性能与大规模数据集训练相当。
...
### [234] [Tailored Architectures for Time Series Forecasting: Evaluating Deep Learning Models on Gaussian Process-Generated Data](https://arxiv.org/abs/2506.08977)
*Victoria Hankemeier,Malte Schilling*

Main category: cs.LG

TL;DR: 论文提出了一种新数据集和模型TimeFlex，用于研究时间序列特性与模型性能的关系，并验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 揭示时间序列特性与特定模型性能之间的明确联系，填补现有研究在模型适应性评估上的不足。

Method: 使用高斯过程生成具有已知特性的新数据集，并开发模块化架构模型TimeFlex，以处理多样化的时间动态。

Result: TimeFlex在多样时间序列条件下表现优异，提供了模型性能的深入理解。

Conclusion: 研究为时间序列预测提供了更清晰的模型选择依据，TimeFlex展示了强大的适应性。
...
### [235] [Propositional Logic for Probing Generalization in Neural Networks](https://arxiv.org/abs/2506.08978)
*Anna Langedijk,Jaap Jumelet,Willem Zuidema*

Main category: cs.LG

TL;DR: 论文研究了三种神经网络架构（Transformers、GCNs、LSTMs）在命题逻辑任务中的泛化能力，发现它们在处理未见过的模式（尤其是否定操作）时表现不佳，表明需要更强的归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 探讨神经网络是否能获取和表示符号规则，特别是在命题逻辑任务中的泛化能力。

Method: 使用扩展的数据集评估三种架构在生成逻辑公式满足赋值任务中的表现，重点关注未见过的操作组合。

Result: 所有模型在训练分布内表现良好，但对未见模式（尤其是否定操作）的泛化能力有限，Transformers需结构偏置才能组合性应用否定。

Conclusion: 标准架构在系统学习逻辑操作表示方面存在局限性，需更强的归纳偏置以支持基于规则的推理。
...
### [236] [On Finetuning Tabular Foundation Models](https://arxiv.org/abs/2506.08982)
*Ivan Rubachev,Akim Kotelnikov,Nikolay Kartashev*

Main category: cs.LG

TL;DR: TabPFNv2是一种新兴的表格深度学习基础模型，研究发现全微调是其最实用的优化策略，能显著提升性能，尤其在I.I.D.数据集上达到最优结果。


<details>
  <summary>Details</summary>
Motivation: 探索如何通过微调优化TabPFNv2的性能，并研究微调如何改变其内部机制。

Method: 系统评估多种微调策略，分析微调对模型内部机制的影响，类比检索增强模型。

Result: 全微调在时间和效果上最优，微调后模型能更准确地衡量测试对象与训练对象的相似性，提升性能。在I.I.D.数据集上达到最优，但在时序变化和复杂特征数据集上表现不稳定。

Conclusion: 全微调是TabPFNv2的最佳实践，尤其在I.I.D.数据集上表现优异，但在复杂场景下仍需改进。
...
### [237] [SwS: Self-aware Weakness-driven Problem Synthesis in Reinforcement Learning for LLM Reasoning](https://arxiv.org/abs/2506.08989)
*Xiao Liang,Zhong-Zhi Li,Yeyun Gong,Yang Wang,Hengyuan Zhang,Yelong Shen,Ying Nian Wu,Weizhu Chen*

Main category: cs.LG

TL;DR: 论文提出了一种自我感知的弱点驱动问题合成框架（SwS），通过识别模型在强化学习中的弱点并针对性生成问题，显著提升了语言模型在复杂推理任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有问题合成方法效率低且未考虑模型能力，导致强化学习效果受限。

Method: 定义模型弱点为核心概念，针对性合成新问题以增强模型弱项。

Result: 在8个主流推理基准测试中，7B和32B模型的平均性能分别提升10.0%和7.7%。

Conclusion: SwS框架无需外部知识蒸馏，通过自我识别和解决弱点实现了鲁棒泛化。
...
### [238] [Branched Schrödinger Bridge Matching](https://arxiv.org/abs/2506.09007)
*Sophia Tang,Yinuo Zhang,Alexander Tong,Pranam Chatterjee*
...
Result: BranchSBM不仅更具表达能力，而且在多路径导航、细胞命运分叉和细胞响应模拟等任务中表现出色。

Conclusion: BranchSBM为解决多路径演化问题提供了有效工具，适用于多种复杂场景。
...
### [239] [Effective Data Pruning through Score Extrapolation](https://arxiv.org/abs/2506.09010)
*Sebastian Schmidt,Prasanga Dhungel,Christoffer Löffler,Björn Nieth,Stephan Günnemann,Leo Schwinn*

Main category: cs.LG

TL;DR: 提出了一种新的重要性分数外推框架，仅需训练少量数据即可预测整个数据集的样本重要性，解决了现有剪枝技术需完整训练的局限性。
...
Method: 引入重要性分数外推框架，基于少量数据训练，采用k近邻和图神经网络预测样本重要性。
...
### [240] [SPEED-RL: Faster Training of Reasoning Models via Online Curriculum Learning](https://arxiv.org/abs/2506.09016)
*Ruiqi Zhang,Daman Arora,Song Mei,Andrea Zanette*

Main category: cs.LG

TL;DR: SPEED是一种自适应在线RL课程，通过选择中等难度的训练示例来提高学习效率，实现2x至6x的训练加速且不影响准确性。


<details>
  <summary>Details</summary>
Motivation: 传统均匀采样提示的方法在训练大型语言模型时计算成本高，效率低下。

Method: 提出SPEED方法，选择性采样中等难度的提示，优化梯度估计器的信噪比。

Result: 实验显示SPEED实现2x至6x的训练加速，无需手动调参且不影响准确性。

Conclusion: SPEED是一种高效、自适应的方法，可无缝集成到标准RL算法中。
...
### [241] [Edit Flows: Flow Matching with Edit Operations](https://arxiv.org/abs/2506.09018)
*Marton Havasi,Brian Karrer,Itai Gat,Ricky T. Q. Chen*
...
<details>
  <summary>Details</summary>
Motivation: 传统非自回归模型在生成变长序列时表现不佳，通常需要强制的固定结构。Edit Flows旨在通过灵活的编辑操作更贴近序列数据的真实结构。

Method: Edit Flows利用连续时间马尔可夫链在序列空间上建模编辑操作，并通过扩展状态空间和辅助变量实现高效训练。

Result: 实验表明，Edit Flows在图像描述生成任务上优于自回归和掩码模型，在文本和代码生成任务上显著优于掩码构造方法。

Conclusion: Edit Flows通过灵活的编辑操作和高效训练方法，为非自回归序列生成提供了一种有效的解决方案。
...
### [242] [e3: Learning to Explore Enables Extrapolation of Test-Time Compute for LLMs](https://arxiv.org/abs/2506.09026)
*Amrith Setlur,Matthew Y. R. Yang,Charlie Snell,Jeremy Greer,Ian Wu,Virginia Smith,Max Simchowitz,Aviral Kumar*

Main category: cs.LG

TL;DR: 论文提出了一种通过测试时扩展（test-time scaling）提升LLM推理能力的方法，重点在于实现外推（extrapolation），即模型在超出训练时的最大token预算时仍能提升性能。通过训练LLM进行上下文探索（in-context exploration），结合三种关键要素（e3配方），显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有推理模型在外推性能上表现不佳，无法充分利用测试时的计算资源。论文旨在通过上下文探索训练方法，使模型在更长思考时间内持续提升性能。

Method: 提出e3配方，包括：（1）结合LLM不对称能力的操作链（如验证与生成）；（2）利用错误轨迹的负梯度强化探索；（3）通过特定设计的课程学习将任务难度与训练token预算耦合。

Result: e3-1.7B模型在AIME'25和HMMT'25上表现最佳，外推能力达到训练token预算的2倍，且在pass@1和pass@k指标上均优于基础模型。

Conclusion: 通过上下文探索训练方法，e3配方显著提升了LLM的外推能力和推理性能，为测试时扩展提供了有效路径。
...
### [243] [FZOO: Fast Zeroth-Order Optimizer for Fine-Tuning Large Language Models towards Adam-Scale Speed](https://arxiv.org/abs/2506.09034)
*Sizhe Dang,Yangyang Guo,Yanjun Zhao,Haishan Ye,Xiaodong Zheng,Guang Dai,Ivor Tsang*

Main category: cs.LG

TL;DR: FZOO是一种快速零阶优化器，显著减少收敛所需的前向传递次数，同时保持与Adam相当的速度，适用于大语言模型微调。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型微调中GPU内存瓶颈问题，现有零阶优化器（如MeZO）收敛速度慢。

Method: FZOO采用批量单边估计和自适应步长调整，结合CUDA并行处理加速计算。

Result: FZOO在11个任务中平均比MeZO准确率提高3%，前向传递次数减少3倍；RoBERTa-large上准确率提升5.6%，前向传递减少18倍。

Conclusion: FZOO实现了单GPU高速全参数微调，为内存高效预训练指明方向。
...
### [244] [The Decoupled Risk Landscape in Performative Prediction](https://arxiv.org/abs/2506.09044)
*Javier Sanguino,Thomas Kehrenberg,Jose A. Lozano,Novi Quadrianto*

Main category: cs.LG

TL;DR: 论文提出了一种可视化方法，用于补充Performative Prediction的理论研究，并引入扩展的Performative Prediction新场景。


<details>
  <summary>Details</summary>
Motivation: 现有文献主要从理论角度研究Performative Prediction，缺乏实践视角。通过可视化损失景观，可以更直观地理解模型与数据的交互。

Method: 提出了一种解耦风险可视化方法，将风险景观分为模型参数和数据参数两部分，并基于此引入了扩展的Performative Prediction场景。

Result: 可视化方法揭示了兴趣点的新特性，并验证了现有算法在更现实条件下的表现。

Conclusion: 可视化方法为Performative Prediction提供了实践补充，扩展的新场景更贴近现实情况。
...
### [245] [Agentic Neural Networks: Self-Evolving Multi-Agent Systems via Textual Backpropagation](https://arxiv.org/abs/2506.09046)
*Xiaowen Ma,Chenyang Lin,Yao Zhang,Volker Tresp,Yunpu Ma*

Main category: cs.LG

TL;DR: Agentic Neural Network（ANN）通过将多智能体协作建模为分层神经网络架构，动态分解任务并优化协作，显著提升了准确性和适应性。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统依赖静态配置，限制了灵活性和性能。ANN旨在通过神经网络的动态性和反馈机制，实现更高效的多智能体协作。

Method: ANN采用两阶段优化策略：前向阶段动态分解任务并构建协作团队；后向阶段通过反馈优化全局和局部协作。

Result: 在四个基准数据集上，ANN在相同配置下优于现有多智能体基线，表现出更高的准确性和适应性。

Conclusion: ANN提供了一个可扩展、数据驱动的多智能体框架，结合了LLM的协作能力和神经网络的高效性，未来将开源。
...
### [246] [Understanding Task Vectors in In-Context Learning: Emergence, Functionality, and Limitations](https://arxiv.org/abs/2506.09048)
*Yuxin Dong,Jiachen Jiang,Zhihui Zhu,Xia Ning*

Main category: cs.LG

TL;DR: 论文提出线性组合猜想，认为任务向量是原始演示的线性组合，并通过理论和实验验证了其有效性。
...
Method: 通过损失景观分析验证任务向量的自然涌现，并预测其在表示高秩映射时的失败。

Result: 实验证实任务向量在高秩映射中表现不佳，但通过注入多个任务向量可提升性能。

Conclusion: 研究深化了对任务向量的理解，揭示了基于Transformer模型的上下文学习机制。
...
### [247] [Stop Misusing t-SNE and UMAP for Visual Analytics](https://arxiv.org/abs/2506.08725)
...
### [248] [Implementing Keyword Spotting on the MCUX947 Microcontroller with Integrated NPU](https://arxiv.org/abs/2506.08911)
...
### [249] [Stop Misusing t-SNE and UMAP for Visual Analytics](https://arxiv.org/abs/2506.08725)
*Hyeon Jeon,Jeongin Park,Sungbok Shin,Jinwook Seo*

Main category: cs.HC

TL;DR: 论文指出t-SNE和UMAP在可视化分析中的误用现象普遍，探讨了原因并提出了改进建议。


<details>
  <summary>Details</summary>
Motivation: 揭示t-SNE和UMAP误用的普遍性及其背后的原因，以促进更合理的使用。

Method: 通过文献综述（114篇论文）和访谈研究，分析误用现象及原因。

Result: 误用主要源于对技术适用性的讨论不足，提出了改进方向。

Conclusion: 建议加强技术适用性讨论，推动更合理的降维技术使用。
...
### [250] [Implementing Keyword Spotting on the MCUX947 Microcontroller with Integrated NPU](https://arxiv.org/abs/2506.08911)
*Petar Jakuš,Hrvoje Džapo*

Main category: cs.HC

TL;DR: 本文介绍了一种在NXP MCXN947微控制器上实现的关键词检测系统，利用集成的NPU实现实时语音交互，并通过量化感知训练优化模型，显著提升推理速度。
...
Method: 结合MFCC特征提取与CNN分类器，采用量化感知训练优化模型，减少模型大小并保持较高准确率。

Result: 实验结果显示，利用NPU比仅用CPU推理速度快59倍，模型大小为30.58 KB，准确率达97.06%。

Conclusion: 该系统证明了在嵌入式平台上实现高效低功耗语音接口的可行性。
...
### [251] [TS-PIELM: Time-Stepping Physics-Informed Extreme Learning Machine Facilitates Soil Consolidation Analyses](https://arxiv.org/abs/2506.08381)
...
### [252] [TS-PIELM: Time-Stepping Physics-Informed Extreme Learning Machine Facilitates Soil Consolidation Analyses](https://arxiv.org/abs/2506.08381)
*He Yang,Fei Ren,Hai-Sui Yu,Xueyu Geng,Pei-Zhi Zhuang*

Main category: physics.geo-ph

TL;DR: 论文提出了一种高效且高精度的物理信息机器学习方法（TS-PIELM），用于改进传统PINN在土壤固结分析中的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统物理信息神经网络（PINN）在土壤固结分析中的精度和效率不足，需要改进。

Method: 提出TS-PIELM框架，将固结过程分为多个时间区间，使用单层前馈极限学习机（ELM）加速训练，直接计算输出层权重。

Result: TS-PIELM的计算效率和精度分别比PINN提高了1000倍和100倍。
...
### [253] [Systematic and Efficient Construction of Quadratic Unconstrained Binary Optimization Forms for High-order and Dense Interactions](https://arxiv.org/abs/2506.08448)
...
### [254] [The interplay of robustness and generalization in quantum machine learning](https://arxiv.org/abs/2506.08455)
...
### [255] [Solving excited states for long-range interacting trapped ions with neural networks](https://arxiv.org/abs/2506.08594)
...
### [256] [Superposed Parameterised Quantum Circuits](https://arxiv.org/abs/2506.08749)
...
### [257] [Systematic and Efficient Construction of Quadratic Unconstrained Binary Optimization Forms for High-order and Dense Interactions](https://arxiv.org/abs/2506.08448)
*Hyakka Nakada,Shu Tanaka*

Main category: quant-ph

TL;DR: 论文提出了一种基于量子退火（QA）解决高阶组合优化问题的方法，通过使用ReLU基函数将目标函数转化为二次无约束二进制优化（QUBO）形式。


<details>
  <summary>Details</summary>
Motivation: 传统二次化方法在处理涉及机器学习的复杂问题时效果不佳，因为这些问题具有强非线性和密集交互性。

Method: 利用ReLU基函数对目标函数建模，并将其转化为等效的二次多项式表示，同时结合QA设计了一种新的黑盒优化方案。

Result: 通过数值和解析方法验证了该方法的可行性，并展示了其在黑盒优化中的应用。

Conclusion: 该方法为复杂机器学习问题的高效二次化提供了新思路，扩展了QA的应用范围。
...
### [258] [The interplay of robustness and generalization in quantum machine learning](https://arxiv.org/abs/2506.08455)
*Julian Berberich,Tobias Fellner,Christian Holm*
...
Method: 通过Lipschitz界量化鲁棒性和泛化性，提出基于模型参数的正则化训练方法。

Result: 理论结果在时间序列分析中得到验证，展示了其实际应用价值。

Conclusion: 强调可训练数据编码策略的重要性，为鲁棒且泛化性强的量子模型提供新思路。
...
### [259] [Solving excited states for long-range interacting trapped ions with neural networks](https://arxiv.org/abs/2506.08594)
*Yixuan Ma,Chang Liu,Weikang Li,Shun-Yao Zhang,L. -M. Duan,Yukai Wu,Dong-Ling Deng*
...
Method: 开发了NQES算法，利用神经网络同时输出多个低激发态，适用于高维系统，并在Haldane-Shastry模型和长程相互作用离子系统中进行了验证。

Result: NQES算法能够高效计算多个激发态及其可观测量期望值，在300离子系统中成功揭示了能隙标度和关联特征。
...
### [260] [Superposed Parameterised Quantum Circuits](https://arxiv.org/abs/2506.08749)
*Viktoria Patapovich,Mo Kordzanganeh,Alexey Melnikov*

Main category: quant-ph

TL;DR: 论文提出了一种叠加参数化量子电路（superposed parameterised quantum circuits），通过结合翻转量子随机存取存储器和重复直到成功协议，提高了量子机器学习的表达能力和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有量子机器学习方法受限于线性酉操作和共享可训练参数，限制了其表达能力和可扩展性，无法与经典深度网络的多层非线性架构相比。

Method: 采用叠加参数化量子电路，结合翻转量子随机存取存储器和重复直到成功协议，嵌入指数级数量的参数化子模型，并通过振幅变换和后选择实现多项式激活函数。

Result: 数值实验表明，该方法在1D阶跃函数回归中比基线模型降低了三个数量级的均方误差；在2D星形分类任务中，引入二次激活函数将准确率提升至81.4%，并减少了运行方差。

Conclusion: 叠加参数化量子电路为构建更深、更通用的参数化量子电路提供了一种硬件高效的途径，能够学习复杂决策边界。
...
### [261] [MOSS: Multi-Objective Optimization for Stable Rule Sets](https://arxiv.org/abs/2506.08030)
...
### [262] [Continuous Policy and Value Iteration for Stochastic Control Problems and Its Convergence](https://arxiv.org/abs/2506.08121)
*Qi Feng,Gu Wang*
...
### [263] [Solving Convex-Concave Problems with $\tilde{\mathcal{O}}(ε^{-4/7})$ Second-Order Oracle Complexity](https://arxiv.org/abs/2506.08362)
...
### [264] [Sharper Convergence Rates for Nonconvex Optimisation via Reduction Mappings](https://arxiv.org/abs/2506.08428)
...
### [265] [Optimization over Sparse Support-Preserving Sets: Two-Step Projection with Global Optimality Guarantees](https://arxiv.org/abs/2506.08558)
...
### [266] [MOSS: Multi-Objective Optimization for Stable Rule Sets](https://arxiv.org/abs/2506.08030)
*Brian Liu,Rahul Mazumder*

Main category: math.OC

TL;DR: MOSS是一个多目标优化框架，用于构建稳定的决策规则集，结合了稀疏性、准确性和稳定性，并通过快速评估权衡来选择合适的模型。


<details>
  <summary>Details</summary>
Motivation: 解决决策规则集中稀疏性、准确性和稳定性之间的权衡问题，提供高效的模型选择方法。

Method: 开发了一种专用的切割平面算法，快速计算Pareto前沿，适用于大规模问题。

Result: MOSS在预测性能和稳定性上优于现有规则集成方法。

Conclusion: MOSS为构建稳定且可解释的决策规则集提供了高效框架。
...
### [267] [Continuous Policy and Value Iteration for Stochastic Control Problems and Its Convergence](https://arxiv.org/abs/2506.08121)
*Qi Feng,Gu Wang*

Main category: math.OC

TL;DR: 提出了一种连续策略-价值迭代算法，通过Langevin型动态同时更新随机控制问题的价值函数和最优控制的近似解。


<details>
  <summary>Details</summary>
Motivation: 解决熵正则化松弛控制问题和经典控制问题中的无限时域优化，同时结合机器学习的分布采样和非凸学习技术。

Method: 利用Langevin型随机微分方程进行连续更新，结合策略迭代方向，同时优化价值函数和最优控制。

Result: 在哈密顿单调性条件下，证明了策略改进和收敛到最优控制。

Conclusion: 该方法为随机控制问题提供了一种高效且通用的解决方案，适用于机器学习的非凸优化场景。
...
### [268] [Solving Convex-Concave Problems with $\tilde{\mathcal{O}}(ε^{-4/7})$ Second-Order Oracle Complexity](https://arxiv.org/abs/2506.08362)
*Lesi Chen,Chengchang Liu,Luo Luo,Jingzhao Zhang*
...
### [269] [Sharper Convergence Rates for Nonconvex Optimisation via Reduction Mappings](https://arxiv.org/abs/2506.08428)
*Evan Markou,Thalaiyasingam Ajanthan,Stephen Gould*

Main category: math.OC

TL;DR: 论文提出了一种利用高维优化问题中最小化器的几何结构（如流形）的框架，通过降维映射改善优化问题的曲率性质，从而加速梯度方法的收敛。


<details>
  <summary>Details</summary>
Motivation: 高维优化问题的最小化器常形成光滑流形，利用这种结构可以去除冗余方向，改善优化问题的性质。

Method: 引入降维映射框架，通过重新参数化参数空间，使其位于解流形上，从而降低目标函数的维度。

Result: 研究表明，设计良好的降维映射能改善目标函数的曲率，使问题更易求解，理论上加速梯度方法的收敛。

Conclusion: 该框架统一了多种利用结构信息加速优化的场景，为实际算法中的性能提升提供了理论解释。
...
### [270] [Optimization over Sparse Support-Preserving Sets: Two-Step Projection with Global Optimality Guarantees](https://arxiv.org/abs/2506.08558)
*William de Vazelhes,Xiao-Tong Yuan,Bin Gu*
...
### [271] [EDINET-Bench: Evaluating LLMs on Complex Financial Tasks using Japanese Financial Statements](https://arxiv.org/abs/2506.08762)
...
### [272] [EDINET-Bench: Evaluating LLMs on Complex Financial Tasks using Japanese Financial Statements](https://arxiv.org/abs/2506.08762)
*Issa Sugiura,Takashi Ishida,Taro Makino,Chieko Tazuke,Takanori Nakagawa,Kosuke Nakago,David Ha*

Main category: q-fin.ST

TL;DR: 论文介绍了EDINET-Bench，一个开源日本金融基准数据集，用于评估大语言模型（LLM）在金融任务中的表现，填补了该领域数据稀缺的空白。


<details>
  <summary>Details</summary>
Motivation: 日本金融数据的稀缺性阻碍了金融分析领域的学术创新，尤其是LLM在该领域的应用和发展。

Method: 通过从日本EDINET系统下载过去10年的年度报告，并自动标注任务标签，构建了EDINET-Bench数据集。
...
### [273] [A Privacy-Preserving Federated Learning Framework for Generalizable CBCT to Synthetic CT Translation in Head and Neck](https://arxiv.org/abs/2506.08654)
...
### [274] [A Privacy-Preserving Federated Learning Framework for Generalizable CBCT to Synthetic CT Translation in Head and Neck](https://arxiv.org/abs/2506.08654)
*Ciro Benito Raggio,Paolo Zaffino,Maria Francesca Spadea*
...
Method: 采用跨机构水平联邦学习框架（FedSynthCT），基于条件生成对抗网络在多中心数据上协同训练。

Result: 联邦模型在多个中心表现出良好的泛化能力，MAE、SSIM和PSNR指标均达到较高水平，且无需额外训练即可适应外部数据集。

Conclusion: 联邦学习为CBCT到sCT合成提供了隐私保护的协作解决方案，无需集中数据共享或站点特定微调。
...
### [275] [MasHost Builds It All: Autonomous Multi-Agent System Directed by Reinforcement Learning](https://arxiv.org/abs/2506.08507)
...
### [276] [MasHost Builds It All: Autonomous Multi-Agent System Directed by Reinforcement Learning](https://arxiv.org/abs/2506.08507)
*Kuo Yang,Xingjie Yang,Linhui Yu,Qing Xu,Yan Fang,Xu Wang,Zhengyang Zhou,Yang Wang*

Main category: cs.MA

TL;DR: MasHost是一个基于强化学习的框架，用于自主和查询自适应的多智能体系统设计，通过图搜索问题和概率采样机制优化智能体角色和交互。


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体系统构建方法依赖人工设计的交互机制或启发式规则，限制了自主能力并引入人为偏见。

Method: 将多智能体系统构建建模为图搜索问题，通过统一的概率采样机制联合采样智能体角色和交互，并引入分层相对策略优化（HRPO）进行多目标优化。

Result: 在六个基准测试中，MasHost表现优于大多数竞争基线，验证了其有效性、效率和结构合理性。

Conclusion: MasHost是首个基于强化学习的自主多智能体系统图构建框架，为复杂任务提供了更自主和高效的解决方案。
...
### [277] [Physics-Informed Teleconnection-Aware Transformer for Global Subseasonal-to-Seasonal Forecasting](https://arxiv.org/abs/2506.08049)
...
### [278] [WWAggr: A Window Wasserstein-based Aggregation for Ensemble Change Point Detection](https://arxiv.org/abs/2506.08066)
...
### [279] [Constrained Pareto Set Identification with Bandit Feedback](https://arxiv.org/abs/2506.08127)
...
### [280] [Model-Free Kernel Conformal Depth Measures Algorithm for Uncertainty Quantification in Regression Models in Separable Hilbert Spaces](https://arxiv.org/abs/2506.08325)
...
### [281] [Asymptotic Normality of Infinite Centered Random Forests -Application to Imbalanced Classification](https://arxiv.org/abs/2506.08548)
...
### [282] [Flexible and Efficient Drift Detection without Labels](https://arxiv.org/abs/2506.08734)
...
### [283] [Physics-Informed Teleconnection-Aware Transformer for Global Subseasonal-to-Seasonal Forecasting](https://arxiv.org/abs/2506.08049)
*Tengfei Lyu,Weijia Zhang,Hao Liu*

Main category: stat.ML

TL;DR: TelePiT是一种新型深度学习架构，通过多尺度物理和遥相关建模提升全球S2S预测性能。


<details>
  <summary>Details</summary>
Motivation: S2S预测因大气系统的混沌性和多尺度复杂交互而具有挑战性，现有方法未能明确建模关键物理过程和遥相关。

Method: TelePiT包含三个关键组件：球谐嵌入、多尺度物理信息神经ODE和遥相关感知Transformer。

Result: 实验表明，TelePiT显著优于现有数据驱动方法和数值天气预报系统，如2米温度RMSE降低57.7%。

Conclusion: TelePiT通过整合多尺度物理和遥相关建模，显著提升了S2S预测的准确性。
...
### [284] [WWAggr: A Window Wasserstein-based Aggregation for Ensemble Change Point Detection](https://arxiv.org/abs/2506.08066)
*Alexander Stepikin,Evgenia Romanenkova,Alexey Zaytsev*

Main category: stat.ML

TL;DR: 本文提出了一种基于Wasserstein距离的新型集成聚合方法WWAggr，用于提升高维变化点检测的性能，并解决了决策阈值选择问题。


<details>
  <summary>Details</summary>
Motivation: 高维数据流中的变化点检测（CPD）面临数据模式复杂性和常见假设不成立的挑战，现有深度神经网络检测器性能仍有提升空间。

Method: 引入WWAggr方法，利用Wasserstein距离进行任务特定的集成聚合，适用于多种深度CPD模型。

Result: WWAggr方法在性能上优于标准聚合技术，并有效解决了决策阈值选择问题。

Conclusion: WWAggr为高维CPD提供了一种更稳健的解决方案，具有广泛适用性和实际价值。
...
### [285] [Constrained Pareto Set Identification with Bandit Feedback](https://arxiv.org/abs/2506.08127)
*Cyrille Kone,Emilie Kaufmann,Laura Richert*

Main category: stat.ML

TL;DR: 本文提出了一种在可行性约束下识别多目标帕累托集的算法，显著优于现有方法，并证明了其样本复杂度的近优性。


<details>
  <summary>Details</summary>
Motivation: 解决在多目标臂问题中识别帕累托集时需满足可行性约束的问题。

Method: 提出了一种固定置信度识别算法，优于传统的两阶段方法和竞赛类算法。

Result: 算法在样本复杂度上表现优异，理论证明其近优性，并通过实验验证。

Conclusion: 所提算法在理论和实验上均表现出色，适用于多目标约束下的帕累托集识别。
...
### [286] [Model-Free Kernel Conformal Depth Measures Algorithm for Uncertainty Quantification in Regression Models in Separable Hilbert Spaces](https://arxiv.org/abs/2506.08325)
*Marcos Matabuena,Rahul Ghosal,Pavlo Mozharovskyi,Oscar Hernan Madrid Padilla,Jukka-Pekka Onnela*

Main category: stat.ML

TL;DR: 论文提出了一种基于条件深度度量的新型模型无关不确定性量化算法，用于定义预测区域，适用于可分离希尔伯特空间中的预测变量和响应变量。


<details>
  <summary>Details</summary>
Motivation: 深度度量在复杂随机对象中定义水平集具有强大能力，但其在回归建模中的应用尚未充分探索。

Method: 结合条件核均值嵌入和集成深度度量，提出算法，并引入保形预测变体以增强实用性。

Result: 算法在仿真研究和实际数字健康应用中表现出色，提供快速收敛率和一致性结果。
...
### [287] [Asymptotic Normality of Infinite Centered Random Forests -Application to Imbalanced Classification](https://arxiv.org/abs/2506.08548)
*Moria Mayala,Erwan Scornet,Charles Tillier,Olivier Wintenberger*

Main category: stat.ML

TL;DR: 本文研究了在类别不平衡数据上训练中心化随机森林（CRF）的理论问题，提出了一种基于重要性采样（IS）的去偏估计器（IS-ICRF），并证明了其在高度不平衡数据中具有方差减少的优势。


<details>
  <summary>Details</summary>
Motivation: 类别不平衡数据在分类任务中常见，但传统方法可能导致分类器偏差。本文旨在通过理论分析解决这一问题。

Method: 使用中心化随机森林（CRF）在重平衡数据集上训练，并通过重要性采样（IS）去偏，提出IS-ICRF估计器。

Result: 证明了IS-ICRF满足中心极限定理（CLT），并在高度不平衡数据中实现了方差减少。
...
### [288] [Flexible and Efficient Drift Detection without Labels](https://arxiv.org/abs/2506.08734)
*Nelvin Tan,Yu-Ching Shih,Dong Yang,Amol Salunkhe*

Main category: stat.ML

TL;DR: 提出了一种无标签环境下的灵活高效的概念漂移检测算法，基于经典统计过程控制，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型广泛用于自动化决策，概念漂移的早期检测至关重要，但现有研究多基于监督任务，标签延迟时检测困难。

Method: 使用经典统计过程控制，设计无标签环境下的概念漂移检测算法，并引入新框架结合先验检测结果。

Result: 在计算限制下，算法统计性能优于现有方法，数值模拟显示良好效果。

Conclusion: 新算法在无标签环境下高效检测概念漂移，性能优于传统方法，适用于大规模数据集。
...
### [289] [CaliciBoost: Performance-Driven Evaluation of Molecular Representations for Caco-2 Permeability Prediction](https://arxiv.org/abs/2506.08059)
...
### [290] [Protriever: End-to-End Differentiable Protein Homology Search for Fitness Prediction](https://arxiv.org/abs/2506.08954)
...
### [291] [CaliciBoost: Performance-Driven Evaluation of Molecular Representations for Caco-2 Permeability Prediction](https://arxiv.org/abs/2506.08059)
*Huong Van Le,Weibin Ren,Junhong Kim,Yukyung Yun,Young Bin Park,Young Jun Kim,Bok Kyung Han,Inho Choi,Jong IL Park,Hwi-Yeol Yun,Jae-Mun Choi*

Main category: q-bio.QM

TL;DR: 通过系统研究八种分子特征表示类型并结合AutoML技术，提升了Caco-2渗透率的预测精度和效率。


<details>
  <summary>Details</summary>
Motivation: Caco-2渗透率是早期药物发现中预测口服吸收的关键指标，需提高计算预测的准确性和效率。

Method: 结合2D/3D描述符、结构指纹和深度学习嵌入，使用AutoML技术预测Caco-2渗透率，并在不同数据集上评估性能。
...
### [292] [Protriever: End-to-End Differentiable Protein Homology Search for Fitness Prediction](https://arxiv.org/abs/2506.08954)
*Ruben Weitzman,Peter Mørch Groth,Lood Van Niekerk,Aoi Otani,Yarin Gal,Debora Marks,Pascal Notin*
...
<details>
  <summary>Details</summary>
Motivation: 传统基于MSA的同源蛋白检索方法计算成本高，且难以处理高度分化的序列或复杂的插入/删除模式。

Method: Protriever通过端到端可微分框架，结合高效的向量搜索，同时学习检索同源序列并训练目标任务。
...
### [293] [Domain Switching on the Pareto Front: Multi-Objective Deep Kernel Learning in Automated Piezoresponse Force Microscopy](https://arxiv.org/abs/2506.08073)
...
### [294] [Mic-hackathon 2024: Hackathon on Machine Learning for Electron and Scanning Probe Microscopy](https://arxiv.org/abs/2506.08423)
...
### [295] [Domain Switching on the Pareto Front: Multi-Objective Deep Kernel Learning in Automated Piezoresponse Force Microscopy](https://arxiv.org/abs/2506.08073)
*Yu Liu,Utkarsh Pratiush,Kamyar Barakati,Hiroshi Funakubo,Ching-Che Lin,Jaegyu Kim,Lane W. Martin,Sergei V. Kalinin*

Main category: cond-mat.mtrl-sci

TL;DR: 提出了一种多目标核学习工作流，用于从高分辨率成像数据中推断铁电极化切换的微观结构规则，揭示了域壁构型和缺陷分布对切换动力学的影响。
...
Conclusion: 该方法不仅支持高通量主动学习，还为理解切换现象的微观结构控制提供了机制性见解，适用于复杂非可微设计空间的探索。
...
### [296] [Mic-hackathon 2024: Hackathon on Machine Learning for Electron and Scanning Probe Microscopy](https://arxiv.org/abs/2506.08423)
*Utkarsh Pratiush,Austin Houston,Kamyar Barakati,Aditya Raghavan,Dasol Yoon,Harikrishnan KP,Zhaslan Baraissov,Desheng Ma,Samuel S. Welborn,Mikolaj Jakowski,Shawn-Patrick Barhorst,Alexander J. Pattison,Panayotis Manganaris,Sita Sirisha Madugula,Sai Venkata Gayathri Ayyagari,Vishal Kennedy,Ralph Bulanadi,Michelle Wang,Kieran J. Pang,Ian Addison-Smith,Willy Menacho,Horacio V. Guzman,Alexander Kiefer,Nicholas Furth,Nikola L. Kolev,Mikhail Petrov,Viktoriia Liu,Sergey Ilyev,Srikar Rairao,Tommaso Rodani,Ivan Pinto-Huguet,Xuli Chen,Josep Cruañes,Marta Torrens,Jovan Pomar,Fanzhi Su,Pawan Vedanti,Zhiheng Lyu,Xingzhi Wang,Lehan Yao,Amir Taqieddin,Forrest Laskowski,Xiangyu Yin,Yu-Tsun Shao,Benjamin Fein-Ashley,Yi Jiang,Vineet Kumar,Himanshu Mishra,Yogesh Paul,Adib Bazgir,Rama chandra Praneeth Madugula,Yuwen Zhang,Pravan Omprakash,Jian Huang,Eric Montufar-Morales,Vivek Chawla,Harshit Sethi,Jie Huang,Lauri Kurki,Grace Guinan,Addison Salvador,Arman Ter-Petrosyan,Madeline Van Winkle,Steven R. Spurgeon,Ganesh Narasimha,Zijie Wu,Richard Liu,Yongtao Liu,Boris Slautin,Andrew R Lupini,Rama Vasudevan,Gerd Duscher,Sergei V. Kalinin*

Main category: cond-mat.mtrl-sci

TL;DR: 论文探讨了显微镜数据管理中的挑战，提出了通过黑客马拉松促进ML与显微镜社区合作，以开发标准化工具和数据集。


<details>
  <summary>Details</summary>
Motivation: 显微镜数据虽丰富但格式不一致，缺乏标准化工具和集成策略，导致分析效率低下。ML与显微镜社区间的隔阂限制了技术进步。

Method: 通过黑客马拉松促进ML研究者与显微镜专家合作，开发标准化代码、基准数据集和数字孪生显微镜。
...
### [297] [Interpreting Agent Behaviors in Reinforcement-Learning-Based Cyber-Battle Simulation Platforms](https://arxiv.org/abs/2506.08192)
...
### [298] [Your Agent Can Defend Itself against Backdoor Attacks](https://arxiv.org/abs/2506.08336)
...
<details>
  <summary>Details</summary>
Motivation: 研究如何通过简化复杂状态和动作空间以及追踪关键事件，提高对深度强化学习代理行为的可解释性。

Method: 通过分析评估场景中的关键事件和状态转换，识别攻击和防御代理的行为模式及动作有效性。

Result: 防御代理通常在1-2个时间步内清除入侵；某些关键动作的有效性仅为1%-60%；诱饵服务可阻止高达94%的特权访问攻击。

Conclusion: 研究揭示了代理行为的可解释性方法，并指出了CAGE Challenge 4对部分问题的改进。
...
### [300] [Your Agent Can Defend Itself against Backdoor Attacks](https://arxiv.org/abs/2506.08336)
*Li Changjiang,Liang Jiacheng,Cao Bochuan,Chen Jinghui,Wang Ting*
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM代理的广泛应用，后门攻击在训练和微调阶段带来的安全风险日益突出，可能导致代理在特定触发条件下执行恶意操作。

Method: ReAgent采用两级检测方法：执行层验证代理思维与行动的一致性；规划层通过代理重建指令的能力检查其与用户指令的一致性。

Result: 实验表明，ReAgent能有效降低攻击成功率（如数据库操作任务中攻击成功率降低90%），显著优于现有防御方法。

Conclusion: ReAgent揭示了利用被攻击代理自身能力缓解后门风险的潜力。
...
### [301] [Learning-Based Multiuser Scheduling in MIMO-OFDM Systems with Hybrid Beamforming](https://arxiv.org/abs/2506.08263)
...
### [302] [Learning-Based Multiuser Scheduling in MIMO-OFDM Systems with Hybrid Beamforming](https://arxiv.org/abs/2506.08263)
*Pouya Agheli,Tugce Kobal,François Durand,Matthew Andrews*

Main category: cs.IT

TL;DR: 研究多用户调度问题，结合MIMO-OFDM和混合波束成形，通过两时间尺度协议和机器学习方法优化比例公平性。


<details>
  <summary>Details</summary>
Motivation: 在毫米波信道中，混合波束成形系统的多路复用增益有限，优化调度对提升频谱效率和长期性能至关重要。

Method: 采用两时间尺度协议：长时尺度分配模拟波束，短时尺度调度用户并设计数字预编码器；提出组合算法（贪婪、排序）和机器学习方法。

Result: 数值结果展示了性能与复杂度的权衡，不同方法适用于不同场景。

Conclusion: 方法选择需根据具体场景标准，混合波束成形和调度策略能有效提升系统性能。
...
### [303] [midr: Learning from Black-Box Models by Maximum Interpretation Decomposition](https://arxiv.org/abs/2506.08338)
...
### [304] [midr: Learning from Black-Box Models by Maximum Interpretation Decomposition](https://arxiv.org/abs/2506.08338)
*Ryoichi Asashiba,Reiji Kozuma,Hirokazu Iwasawa*

Main category: stat.ME

TL;DR: 论文介绍了一个名为midr的R包，用于实现最大解释分解（MID），旨在通过功能分解方法解释黑盒模型。
...
Result: midr包能够从黑盒模型中学习，并提供高级分析功能。
...
### [305] [Bridging RDF Knowledge Graphs with Graph Neural Networks for Semantically-Rich Recommender Systems](https://arxiv.org/abs/2506.08743)
...
### [306] [Bridging RDF Knowledge Graphs with Graph Neural Networks for Semantically-Rich Recommender Systems](https://arxiv.org/abs/2506.08743)
*Michael Färber,David Lamprecht,Yuni Susanti*
...
Method: 提出了一种综合集成方法，利用RDF对象属性的拓扑信息和数据类型属性的内容信息，并评估不同GNN的性能。
...
### [307] [A Metrics-Oriented Architectural Model to Characterize Complexity on Machine Learning-Enabled Systems](https://arxiv.org/abs/2506.08153)
...
### [308] [On The Impact of Merge Request Deviations on Code Review Practices](https://arxiv.org/abs/2506.08860)
...
### [309] [A Metrics-Oriented Architectural Model to Characterize Complexity on Machine Learning-Enabled Systems](https://arxiv.org/abs/2506.08153)
*Renato Cordeiro Ferreira*

Main category: cs.SE

TL;DR: 研究提出基于指标的架构模型，以管理ML系统的复杂性。
...
TL;DR: 论文提出了一种识别代码审查中非标准合并请求（MR）的方法，并证明排除这些偏差能提升机器学习模型的性能。


<details>
  <summary>Details</summary>
Motivation: 工业中的合并请求（MR）流程常偏离标准审查流程，导致分析和机器学习模型存在偏差。

Method: 提出了七种偏差类别，并开发了一种少样本学习检测方法（准确率91%）。

Result: 排除偏差后，预测审查完成时间的ML模型性能提升53.33%，特征重要性显著变化。

Conclusion: 研究为优化代码审查提供了偏差分类、检测方法及其对ML分析影响的实证支持。
...
### [311] [TIP-Search: Time-Predictable Inference Scheduling for Market Prediction under Uncertain Load](https://arxiv.org/abs/2506.08026)
*Xibai Wang*
...
### [312] [SafeCoT: Improving VLM Safety with Minimal Reasoning](https://arxiv.org/abs/2506.08399)
...
### [313] [FEDTAIL: Federated Long-Tailed Domain Generalization with Sharpness-Guided Gradient Matching](https://arxiv.org/abs/2506.08518)
...
### [314] [Paths to Causality: Finding Informative Subgraphs Within Knowledge Graphs for Knowledge-Based Causal Discovery](https://arxiv.org/abs/2506.08771)
...
### [315] [IntTrajSim: Trajectory Prediction for Simulating Multi-Vehicle driving at Signalized Intersections](https://arxiv.org/abs/2506.08957)
...
### [316] [TIP-Search: Time-Predictable Inference Scheduling for Market Prediction under Uncertain Load](https://arxiv.org/abs/2506.08026)
*Xibai Wang*

Main category: cs.AI

TL;DR: TIP-Search是一个时间可预测的推理调度框架，用于在不确定工作负载下进行实时市场预测，动态选择深度学习模型以满足严格延迟需求。


<details>
  <summary>Details</summary>
Motivation: 高频金融系统对延迟有严格要求，需要一种能够在满足任务截止时间的同时最大化预测准确性的方法。

Method: TIP-Search通过离线分析延迟和泛化性能，在线进行任务感知选择，无需显式输入域标签。

Result: 在三个真实数据集上测试，TIP-Search比静态基线方法准确率提升8.5%，且100%满足截止时间。
...
### [317] [SafeCoT: Improving VLM Safety with Minimal Reasoning](https://arxiv.org/abs/2506.08399)
*Jiachen Ma,Zhanhui Zhou,Chao Yang,Chaochao Lu*

Main category: cs.AI

TL;DR: SafeCoT是一个轻量级、可解释的框架，通过规则引导的思维链监督改进视觉语言模型的安全拒绝行为，减少过度拒绝并提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在高风险或模糊场景中的安全响应仍是一个关键挑战，需要一种轻量且可扩展的解决方案。

Method: SafeCoT利用规则引导的思维链监督，通过最小化监督帮助模型推理安全风险并做出上下文感知的拒绝。

Result: 实验表明，SafeCoT显著减少了过度拒绝并提升了泛化能力，即使在有限训练数据下也表现良好。
...
### [318] [FEDTAIL: Federated Long-Tailed Domain Generalization with Sharpness-Guided Gradient Matching](https://arxiv.org/abs/2506.08518)
*Sunny Gupta,Nikita Jangid,Shounak Das,Amit Sethi*
...
<details>
  <summary>Details</summary>
Motivation: 解决领域泛化中长尾类分布和优化目标冲突的问题，提升模型在未见目标域上的性能。

Method: 采用梯度一致性正则化减少分类与对抗目标的冲突，类感知锐度最小化及动态加权方案处理类别不平衡，结合锐度感知扰动增强条件分布对齐。

Result: 在标准领域泛化基准测试中，FedTAIL表现最优，尤其在领域偏移和标签不平衡情况下。

Conclusion: FedTAIL通过优化协调、类感知正则化和条件对齐的统一框架，在集中式和联邦设置下均有效提升泛化性能。
...
### [319] [Paths to Causality: Finding Informative Subgraphs Within Knowledge Graphs for Knowledge-Based Causal Discovery](https://arxiv.org/abs/2506.08771)
*Yuni Susanti,Michael Färber*

Main category: cs.AI

TL;DR: 论文提出了一种结合知识图谱（KGs）与大语言模型（LLMs）的新方法，以提升基于知识的因果发现，解决了现有LLMs方法结果不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于观测数据的因果发现方法存在局限性，而基于知识的因果发现（如利用变量元数据）是一种有潜力的替代方案，但现有LLMs方法结果不稳定，影响可靠性。

Method: 通过知识图谱中的元路径子图识别，结合学习排序模型优化子图选择，并将排名靠前的子图融入零样本提示，提升LLMs的因果推理能力。

Result: 在生物医学和开放领域数据集上的实验表明，该方法在F1分数上比基线方法最高提升44.4分。

Conclusion: 结合KGs与LLMs的方法显著提升了基于知识的因果发现的稳定性和效果，为复杂系统中的因果推理提供了可靠工具。
...
### [320] [IntTrajSim: Trajectory Prediction for Simulating Multi-Vehicle driving at Signalized Intersections](https://arxiv.org/abs/2506.08957)
*Yash Ranjan,Rahul Sengupta,Anand Rangarajan,Sanjay Ranka*

Main category: cs.AI

TL;DR: 论文提出了一种基于数据驱动的交通模拟器，通过深度生成模型模拟交通路口的驾驶行为，并引入交通工程相关指标进行评估。


<details>
  <summary>Details</summary>
Motivation: 传统基于规则的交通模拟器难以真实模拟驾驶行为，而交通路口在安全和效率方面至关重要，因此需要数据驱动的模拟器。

Method: 提出了一种多头自注意力轨迹预测模型，结合信号信息，并通过模拟闭环管道评估生成模型。

Result: 新模型在交通工程相关指标上优于之前的模型。

Conclusion: 研究为交通路口的数据驱动模拟提供了有效方法和评估框架。
...
### [321] [syren-baryon: Analytic emulators for the impact of baryons on the matter power spectrum](https://arxiv.org/abs/2506.08783)
...
### [322] [syren-baryon: Analytic emulators for the impact of baryons on the matter power spectrum](https://arxiv.org/abs/2506.08783)
*Lukas Kammerer,Deaglan J. Bartlett,Gabriel Kronberger,Harry Desmond,Pedro G. Ferreira*

Main category: astro-ph.CO

TL;DR: 论文提出了一种符号回归方法，用于构建重子物理对物质功率谱影响的解析近似，并提供了不同重子物理模型的预测及其不确定性。


<details>
  <summary>Details</summary>
Motivation: 重子物理对宇宙物质分布的显著影响是当前和未来宇宙学调查的关键系统误差来源，需要简单且物理合理的参数化描述。

Method: 使用符号回归从CAMELS流体动力学模拟中提取重子物理对物质功率谱影响的解析近似，涵盖四种不同子网格模型和一个重子化算法。

Result: 解析近似的误差与样本方差相当，重子化表达式的均方根误差小于1%，且在大尺度和高红移下具有物理正确性。

Conclusion: 这些解析形式可直接解释宇宙学和反馈参数的影响，并可用于区分不同重子物理模型，相关代码已公开。
...
### [323] [Evaluation of Machine Learning Models in Student Academic Performance Prediction](https://arxiv.org/abs/2506.08047)
...
### [324] [Evaluation of Machine Learning Models in Student Academic Performance Prediction](https://arxiv.org/abs/2506.08047)
*A. G. R. Sandeepa,Sanka Mohottala*
...
<details>
  <summary>Details</summary>
Motivation: 探索机器学习，尤其是神经网络，在预测学生学业表现方面的潜力，并验证其数据效率。

Method: 使用学生行为、学术和人口统计数据，采用多层感知机分类器（MLPC）等标准机器学习模型，结合特征选择方法。

Result: MLPC在测试集上最高准确率达86.46%，10折交叉验证平均准确率为79.58%，训练集准确率高达99.65%。

Conclusion: 神经网络（如MLPC）在预测学生表现方面具有潜力，特征选择对性能提升至关重要，可解释性方法有助于验证模型。
...
### [325] [Dynamic Diffusion Schrödinger Bridge in Astrophysical Observational Inversions](https://arxiv.org/abs/2506.08065)
...
### [326] [Dynamic Diffusion Schrödinger Bridge in Astrophysical Observational Inversions](https://arxiv.org/abs/2506.08065)
*Ye Zhu,Duo Xu,Zhiwei Deng,Jonathon C. Tan,Olga Russakovsky*

Main category: astro-ph.IM

TL;DR: Astro-DSB模型是一种针对天体物理动力学设计的扩散薛定谔桥模型，在GMC中用于恒星形成的观测逆预测任务，优于传统方法和生成模型。


<details>
  <summary>Details</summary>
Motivation: 研究扩散模型在天体物理系统中的应用，解决GMC中恒星形成的观测逆预测问题，提升模型的解释性和学习效率。

Method: 提出Astro-DSB模型，基于成对域假设，通过物理模拟和真实观测数据（Taurus B213）验证其性能。

Result: Astro-DSB在解释性、学习效率和预测性能上优于传统方法，且在OOD测试中表现更优。

Conclusion: 研究扩展了扩散模型的应用范围，展示了其在物理系统学习中的潜力，为未来物理感知生成模型铺路。
...
### [327] [LEANN: A Low-Storage Vector Index](https://arxiv.org/abs/2506.08276)
...
### [328] [LEANN: A Low-Storage Vector Index](https://arxiv.org/abs/2506.08276)
*Yichuan Wang,Shu Liu,Zhifei Li,Yongji Wu,Ziming Mao,Yilong Zhao,Xiao Yan,Zhiying Xu,Yang Zhou,Ion Stoica,Sewon Min,Matei Zaharia,Joseph E. Gonzalez*

Main category: cs.DB

TL;DR: LEANN是一种针对资源受限设备的存储高效近似最近邻搜索索引，显著减少存储开销，同时保持搜索质量和延迟。


<details>
  <summary>Details</summary>
Motivation: 本地设备上支持基于嵌入的搜索需求增长，但传统方法存储开销过高，难以实现。

Method: LEANN结合紧凑的图结构和动态重计算策略，实现快速准确检索。

Result: LEANN将索引大小降至原始数据的5%，存储减少50倍，保持90%的top-3召回率，延迟低于2秒。

Conclusion: LEANN为资源受限设备提供了一种高效的解决方案，显著降低了存储需求。
...
### [329] [Werewolf: A Straightforward Game Framework with TTS for Improved User Engagement](https://arxiv.org/abs/2506.00160)
...
### [330] [Multilingual Hate Speech Detection in Social Media Using Translation-Based Approaches with Large Language Models](https://arxiv.org/abs/2506.08147)
...
### [331] [mSTEB: Massively Multilingual Evaluation of LLMs on Speech and Text Tasks](https://arxiv.org/abs/2506.08400)
...
### [332] [Low-resource domain adaptation while minimizing energy and hardware resource consumption](https://arxiv.org/abs/2506.08433)
...
### [333] [Olica: Efficient Structured Pruning of Large Language Models without Retraining](https://arxiv.org/abs/2506.08436)
...
### [334] [DRAGged into Conflicts: Detecting and Addressing Conflicting Sources in Search-Augmented LLMs](https://arxiv.org/abs/2506.08500)
...
### [335] [CoMuMDR: Code-mixed Multi-modal Multi-domain corpus for Discourse paRsing in conversations](https://arxiv.org/abs/2506.08504)
...
### [336] [Dense Retrievers Can Fail on Simple Queries: Revealing The Granularity Dilemma of Embeddings](https://arxiv.org/abs/2506.08592)
...
### [337] [TableDreamer: Progressive and Weakness-guided Data Synthesis from Scratch for Table Instruction Tuning](https://arxiv.org/abs/2506.08646)
...
### [338] [ConfPO: Exploiting Policy Model Confidence for Critical Token Selection in Large Language Model Preference Optimization](https://arxiv.org/abs/2506.08712)
...
### [339] [Towards Secure and Private Language Models for Nuclear Power Plants](https://arxiv.org/abs/2506.08746)
...
### [340] [Enhancing Accuracy and Maintainability in Nuclear Plant Data Retrieval: A Function-Calling LLM Approach Over NL-to-SQL](https://arxiv.org/abs/2506.08757)
...
### [341] [AdversariaL attacK sAfety aLIgnment(ALKALI): Safeguarding LLMs through GRACE: Geometric Representation-Aware Contrastive Enhancement- Introducing Adversarial Vulnerability Quality Index (AVQI)](https://arxiv.org/abs/2506.08885)
...
### [342] [PropMEND: Hypernetworks for Knowledge Propagation in LLMs](https://arxiv.org/abs/2506.08920)
...
### [343] [Can A Gamer Train A Mathematical Reasoning Model?](https://arxiv.org/abs/2506.08935)
*Andrew Shin*
...
### [344] [Pre-trained Language Models Learn Remarkably Accurate Representations of Numbers](https://arxiv.org/abs/2506.08966)
...
### [345] [Router-R1: Teaching LLMs Multi-Round Routing and Aggregation via Reinforcement Learning](https://arxiv.org/abs/2506.09033)
...
### [346] [Werewolf: A Straightforward Game Framework with TTS for Improved User Engagement](https://arxiv.org/abs/2506.00160)
*Qihui Fan,Enfu Nan,Wenbo Li,Lei Lu,Pu Zhao,Yanzhi Wang*
...
Method: 提出了一种新颖且直接的基于LLM的狼人杀系统，结合优化的TTS模型，无需额外组件。

Result: 系统通过优化的TTS模型提升了与多种LLM模型的兼容性，并改善了用户参与度。

Conclusion: 随着LLM推理能力的持续增强，狼人杀游戏中的额外组件将变得多余。
...
### [347] [Multilingual Hate Speech Detection in Social Media Using Translation-Based Approaches with Large Language Models](https://arxiv.org/abs/2506.08147)
*Muhammad Usman,Muhammad Ahmad,M. Shahiki Tash,Irina Gelbukh,Rolando Quintero Tellez,Grigori Sidorov*

Main category: cs.CL

TL;DR: 该论文提出了一个多语言仇恨言论检测框架，通过注意力层和大型语言模型（如GPT-3.5 Turbo和Qwen 2.5 72B）提升性能，并在英语、西班牙语和乌尔都语数据集上取得了显著改进。
...
Method: 使用注意力层和大型语言模型（如GPT-3.5 Turbo和Qwen 2.5 72B）进行特征提取，并在多语言数据集上进行基准测试。

Result: 在英语、西班牙语和乌尔都语数据集上，F1分数分别达到0.87、0.85和0.81，多语言模型F1分数为0.88。
...
### [348] [mSTEB: Massively Multilingual Evaluation of LLMs on Speech and Text Tasks](https://arxiv.org/abs/2506.08400)
*Luel Hagos Beyene,Vivek Verma,Min Ma,Jesujoba O. Alabi,Fabian David Schmidt,Joyce Nakatumba-Nabende,David Ifeoluwa Adelani*
...
### [349] [Low-resource domain adaptation while minimizing energy and hardware resource consumption](https://arxiv.org/abs/2506.08433)
*Hernán Maina,Nicolás Wolovick,Luciana Benotti*
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练成本高且存在文化偏见，领域适应虽有效但计算成本高，尤其在资源有限的环境中。

Method: 评估不同数值精度和数据并行化策略对训练速度和模型精度的影响。

Result: 研究结果对能源效率、可访问性或硬件资源有限的环境具有重要意义。
...
### [350] [Olica: Efficient Structured Pruning of Large Language Models without Retraining](https://arxiv.org/abs/2506.08436)
*Jiujun He,Huazhen Lin*

Main category: cs.CL

TL;DR: Olica是一种无需重新训练的LLM剪枝框架，通过正交分解和线性校准压缩模型，保持精度。
...
Result: Olica在数据使用、GPU内存和运行时间上高效，性能优于基准。
...
### [351] [DRAGged into Conflicts: Detecting and Addressing Conflicting Sources in Search-Augmented LLMs](https://arxiv.org/abs/2506.08500)
*Arie Cattan,Alon Jacovi,Ori Ram,Jonathan Herzig,Roee Aharoni,Sasha Goldshtein,Eran Ofek,Idan Szpektor,Avi Caciularu*

Main category: cs.CL

TL;DR: 该论文提出了RAG中知识冲突的分类法，并创建了CONFLICTS基准测试，用于评估LLMs在解决冲突时的表现。实验表明，LLMs在解决冲突时表现不佳，但通过提示其显式推理可改善结果。
...
Result: LLMs在解决冲突时表现不佳，但显式推理提示能显著改善结果。

Conclusion: 未来研究仍需改进LLMs在解决知识冲突方面的能力。
...
### [352] [CoMuMDR: Code-mixed Multi-modal Multi-domain corpus for Discourse paRsing in conversations](https://arxiv.org/abs/2506.08504)
*Divyaksh Shukla,Ritesh Baviskar,Dwijesh Gohil,Aniket Tiwari,Atul Shree,Ashutosh Modi*

Main category: cs.CL

TL;DR: 论文介绍了CoMuMDR语料库，用于多模态多领域代码混合对话的篇章解析，并展示了现有模型的性能不足。


<details>
  <summary>Details</summary>
Motivation: 当前篇章解析数据集局限于单一领域的英文对话，缺乏多模态和代码混合的语料库。

Method: 构建了包含印地语和英语代码混合的多模态语料库CoMuMDR，标注了九种篇章关系，并测试了多种基线模型。
...
### [353] [Dense Retrievers Can Fail on Simple Queries: Revealing The Granularity Dilemma of Embeddings](https://arxiv.org/abs/2506.08592)
*Liyan Xu,Zhenlin Su,Mo Yu,Jiangnan Li,Fandong Meng,Jie Zhou*

Main category: cs.CL

TL;DR: 本文研究了文本编码器在识别细粒度实体或事件时的局限性，提出了中文评估数据集CapRetrieval，并通过微调编码器提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本编码器在细粒度语义匹配上表现不佳，导致密集检索失败，需改进。
...
### [354] [TableDreamer: Progressive and Weakness-guided Data Synthesis from Scratch for Table Instruction Tuning](https://arxiv.org/abs/2506.08646)
*Mingyu Zheng,Zhifan Feng,Jia Wang,Lanrui Wang,Zheng Lin,Yang Hao,Weiping Wang*

Main category: cs.CL

TL;DR: 论文提出了一种名为TableDreamer的框架，通过渐进式和弱点引导的数据合成方法，解决了现有LLM在表指令调优数据生成中的多样性和效率问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在表指令调优数据生成中存在输入空间探索不足和数据效率低下的问题。

Method: TableDreamer首先生成多样化的种子数据，然后基于新发现的弱点数据迭代探索输入空间，最终生成用于微调目标LLM的训练数据。

Result: 在10个表格基准测试中，TableDreamer显著提升了目标LLM的性能（平均准确率提升11.62%），且优于使用更多训练数据的基线方法。
...
### [355] [ConfPO: Exploiting Policy Model Confidence for Critical Token Selection in Large Language Model Preference Optimization](https://arxiv.org/abs/2506.08712)
*Hee Suk Yoon,Eunseop Yoon,Mark A. Hasegawa-Johnson,Sungwoong Kim,Chang D. Yoo*

Main category: cs.CL

TL;DR: ConfPO是一种用于大型语言模型（LLM）偏好学习的方法，通过仅基于训练策略的置信度识别和优化偏好关键令牌，无需辅助模型或额外计算。


<details>
  <summary>Details</summary>
Motivation: 现有直接对齐算法（如DPO）对所有令牌进行统一调整，而ConfPO专注于对偏好影响最大的令牌进行优化，以提高对齐质量并避免过优化。

Method: ConfPO通过KL散度预算更高效地优化关键令牌，无需依赖信用分配模型或人工标注，方法简单轻量且无需额外模型。

Result: 在AlpacaEval 2和Arena-Hard等基准测试中，ConfPO表现优于统一调整的DAA方法，对齐效果更好且无额外计算开销。

Conclusion: ConfPO通过聚焦关键令牌优化，提供了一种高效、轻量的偏好学习方法，显著提升了LLM的对齐效果。
...
### [356] [Towards Secure and Private Language Models for Nuclear Power Plants](https://arxiv.org/abs/2506.08746)
*Muhammad Anwar,Mishca de Costa,Issam Hammad,Daniel Lau*

Main category: cs.CL

TL;DR: 本文介绍了一种针对核应用领域的特定大型语言模型，基于公开的Essential CANDU教材构建，采用紧凑的Transformer架构，在单GPU上训练以保护核操作中的敏感数据。模型在捕获专业核词汇方面表现良好，但生成文本有时缺乏句法连贯性。


<details>
  <summary>Details</summary>
Motivation: 开发一种符合严格网络安全和数据保密标准的内部LLM解决方案，以满足核应用领域的特定需求。

Method: 基于Transformer架构，使用公开的Essential CANDU教材作为训练数据，在单GPU上进行训练。

Result: 模型能够捕获专业核词汇，但生成文本的句法连贯性有待提高。初步展示了在专业任务中的实用性。

Conclusion: 未来需扩展数据集覆盖更多核子领域，优化预处理和指令微调以提升准确性，并评估模型在核领域的实际应用准备度。
...
### [357] [Enhancing Accuracy and Maintainability in Nuclear Plant Data Retrieval: A Function-Calling LLM Approach Over NL-to-SQL](https://arxiv.org/abs/2506.08757)
*Mishca de Costa,Muhammad Anwar,Dave Mercier,Mark Randall,Issam Hammad*

Main category: cs.CL

TL;DR: 论文提出了一种基于函数调用的大型语言模型（LLM）的替代方案，以解决传统自然语言转SQL（NL-to-SQL）在核电站数据查询中的风险。


<details>
  <summary>Details</summary>
Motivation: 核电站操作数据的查询需要极高的准确性和透明度，传统NL-to-SQL方法存在验证困难和复杂数据库结构的问题，导致不准确和信任缺失。

Method: 通过预定义一组经过批准的、特定用途的函数来封装已验证的SQL逻辑，避免直接生成SQL查询，从而降低风险。

Result: 研究表明，基于函数的方法在准确性和可维护性上优于直接NL-to-SQL生成。

Conclusion: 该工作平衡了用户易用性与操作安全性，为关键系统提供了稳健的数据检索框架。
...
### [358] [AdversariaL attacK sAfety aLIgnment(ALKALI): Safeguarding LLMs through GRACE: Geometric Representation-Aware Contrastive Enhancement- Introducing Adversarial Vulnerability Quality Index (AVQI)](https://arxiv.org/abs/2506.08885)
*Danush Khanna,Krishna Kumar,Basab Ghosh,Vinija Jain,Vasu Sharma,Aman Chadha,Amitava Das*

Main category: cs.CL

TL;DR: 论文揭示了LLM对抗性威胁的几何盲点，提出ALKALI基准和GRACE框架以提升防御能力。


<details>
  <summary>Details</summary>
Motivation: 当前防御方法无法应对快速增长的对抗性威胁，尤其是针对LLM的潜在伪装攻击。

Method: 引入ALKALI基准评估21种LLM，提出GRACE框架结合偏好学习和潜在空间正则化。

Result: GRACE显著降低攻击成功率（ASR）达39%，并开发AVQI指标量化潜在对齐失败。

Conclusion: GRACE和AVQI为LLM安全提供了新的防御和评估工具。
...
### [359] [PropMEND: Hypernetworks for Knowledge Propagation in LLMs](https://arxiv.org/abs/2506.08920)
*Zeyu Leo Liu,Greg Durrett,Eunsol Choi*

Main category: cs.CL

TL;DR: PropMEND是一种基于超网络的方法，用于改进大型语言模型的知识传播能力，使其能够回答涉及注入知识的推理问题。
...
Method: 通过元学习修改语言建模损失的梯度，以促进知识传播，扩展了MEND的元目标。

Result: 在RippleEdit数据集上表现优异，多跳问题准确率提升近2倍；在Controlled RippleEdit数据集上仍优于现有方法。

Conclusion: PropMEND在知识传播方面表现优越，但在未见实体关系上仍有改进空间。
...
### [360] [Can A Gamer Train A Mathematical Reasoning Model?](https://arxiv.org/abs/2506.08935)
*Andrew Shin*

Main category: cs.CL

TL;DR: 论文提出了一种在单张游戏GPU上训练高性能数学推理模型的方法，通过结合强化学习和内存优化技术，显著降低了训练成本。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在数学推理等任务上表现优异，但其训练通常需要高昂的计算资源。本文旨在证明，通过优化技术，可以在资源有限的环境中实现高性能。

Method: 结合强化学习和内存优化技术，在单张RTX 3080 Ti（16GB内存）上训练了一个1.5B参数的数学推理模型。

Result: 该模型在数学推理基准测试中表现优于或接近更大规模的模型，证明了资源受限环境下高性能AI研究的可行性。

Conclusion: 研究挑战了高性能数学推理需要大规模基础设施的传统观念，为资源有限的研究者提供了新的可能性。
...
### [361] [Pre-trained Language Models Learn Remarkably Accurate Representations of Numbers](https://arxiv.org/abs/2506.08966)
*Marek Kadlčík,Michal Štefánik,Timothee Mickus,Michal Spiegel,Josef Kuchař*

Main category: cs.CL

TL;DR: 预训练语言模型（LM）在算术任务中容易出错，现有方法难以准确探测数值嵌入。本文提出一种新探测技术，能高精度解码数值，证明LM在预训练后能精确表示数字。


<details>
  <summary>Details</summary>
Motivation: 预训练语言模型在算术任务中表现不佳，现有探测方法未能捕捉到数值嵌入的正弦模式结构。

Method: 提出一种新型探测技术，从输入嵌入中解码数值，验证LM预训练后对数字的精确表示能力。

Result: 新探测技术能近乎完美地解码数值，且嵌入的精确度解释了LM在基础算术中的大部分错误。
...
### [362] [Router-R1: Teaching LLMs Multi-Round Routing and Aggregation via Reinforcement Learning](https://arxiv.org/abs/2506.09033)
*Haozhen Zhang,Tao Feng,Jiaxuan You*
...
Conclusion: Router-R1通过强化学习实现了多LLM的动态路由与聚合，展示了性能与成本权衡的潜力。
...
### [363] [Neural-Augmented Kelvinlet: Real-Time Soft Tissue Deformation with Multiple Graspers](https://arxiv.org/abs/2506.08043)
...
### [364] [Neural-Augmented Kelvinlet: Real-Time Soft Tissue Deformation with Multiple Graspers](https://arxiv.org/abs/2506.08043)
*Ashkan Shahbazi,Kyvia Pereira,Jon S. Heiselman,Elaheh Akbari,Annie C. Benson,Sepehr Seifi,Xinyuan Liu,Garrison L. Johnston,Erwin Terpstra,Anne Draaisma,Jan-Jaap Severes,Jie Ying Wu,Nabil Simaan,Michael L. Miga,Soheil Kolouri*

Main category: cs.GR

TL;DR: 提出了一种基于物理信息的神经模拟器，利用Kelvinlet先验和FEM模拟，实现了高精度、低延迟的软组织变形实时模拟。
...
Result: 实现了高保真度的实时软组织模拟，适用于腹腔镜手术操作。

Conclusion: Kelvinlet增强学习是一种高效且物理感知的软组织模拟策略。
...
### [365] [PerfTracker: Online Performance Troubleshooting for Large-scale Model Training in Production](https://arxiv.org/abs/2506.08528)
...
### [366] [PerfTracker: Online Performance Troubleshooting for Large-scale Model Training in Production](https://arxiv.org/abs/2506.08528)
*Yu Guan,Zhiyu Yin,Haoyu Chen,Sheng Cheng,Chaojie Yang,Tianyin Xu,Yang Zhang,Hanyu Zhao,Yong Li,Dennis Cai,Ennan Zhai*

Main category: cs.DC

TL;DR: PerfTracker是一个在线性能诊断系统，用于解决大规模模型训练中的性能问题，通过细粒度分析和差异可观测性定位问题根源。
...
Method: PerfTracker利用细粒度分析和在线性能分析，结合差异可观测性，诊断硬件和软件层面的性能问题。

Result: PerfTracker已部署在万级GPU集群中，成功诊断多种复杂性能问题。

Conclusion: PerfTracker为大规模模型训练提供了一种高效的性能问题诊断解决方案。
...
### [367] [Structured Variational $D$-Decomposition for Accurate and Stable Low-Rank Approximation](https://arxiv.org/abs/2506.08535)
*Ronald Katende*
...
### [368] [sparseGeoHOPCA: A Geometric Solution to Sparse Higher-Order PCA Without Covariance Estimation](https://arxiv.org/abs/2506.08670)
...
### [369] [Structured Variational $D$-Decomposition for Accurate and Stable Low-Rank Approximation](https://arxiv.org/abs/2506.08535)
*Ronald Katende*

Main category: math.NA

TL;DR: 论文提出了一种非正交矩阵分解方法$D$-decomposition，通过最小化正则化Frobenius损失实现，支持控制秩、稀疏性和条件数，计算复杂度为$\mathcal{O}(n^2k)$，在多个数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统矩阵分解方法（如LU或SVD）在控制秩、稀疏性和条件数方面存在局限性，需要一种更灵活且计算高效的非正交分解方法。

Method: 提出$D$-decomposition，形式为$A \approx P D Q$，通过交替最小化正则化Frobenius损失实现分解，支持对秩、稀疏性和条件数的控制。

Result: 实验证明该方法在MovieLens、MNIST、Olivetti Faces和基因表达矩阵上，尤其在稀疏性和噪声条件下，重建精度优于截断SVD、CUR和非负矩阵分解。

Conclusion: $D$-decomposition是一种高效且灵活的非正交矩阵分解方法，适用于多种实际场景，尤其在稀疏和噪声数据中表现优异。
...
### [370] [sparseGeoHOPCA: A Geometric Solution to Sparse Higher-Order PCA Without Covariance Estimation](https://arxiv.org/abs/2506.08670)
*Renjie Xu,Chong Wu,Maolin Che,Zhuoheng Ran,Yimin Wei,Hong Yan*

Main category: math.NA

TL;DR: 提出了一种名为sparseGeoHOPCA的新框架，通过几何视角解决高维张量分解问题，显著提升计算效率和可解释性。
...
Result: 理论证明了几何子问题与原始SHOPCA公式的等价性，并基于经典PCA残差推导了最坏情况近似误差界限。实验显示在合成设置中准确恢复稀疏支持，并在压缩和图像重建中表现优异。

Conclusion: sparseGeoHOPCA在计算效率和可解释性上表现优异，适用于高维和不平衡数据场景。
...
### [371] [Instruction-Tuned Video-Audio Models Elucidate Functional Specialization in the Brain](https://arxiv.org/abs/2506.08277)
...
### [372] [Instruction-Tuned Video-Audio Models Elucidate Functional Specialization in the Brain](https://arxiv.org/abs/2506.08277)
*Subba Reddy Oota,Khushbu Pahwa,Prachi Jindal,Satya Sai Srinath Namburi,Maneesh Singh,Tanmoy Chakraborty,Bapi S. Raju,Manish Gupta*
...
<details>
  <summary>Details</summary>
Motivation: 填补现有研究在评估MLLMs与大脑对齐性时未充分探索指令调优模型在多媒体刺激下的表现的空白。

Method: 利用13个视频任务指令调优的MLLMs，测量其表征对自然电影观看时神经活动的预测能力。

Result: 指令调优的MLLMs在视频任务中表现优于非指令调优模型15%，在音频任务中表现优于单模态模型20%。

Conclusion: 任务特定指令显著提升MLLMs与大脑活动的对齐性，为研究大脑与模型的信息处理提供了新方向。
...
### [373] [Re4MPC: Reactive Nonlinear MPC for Multi-model Motion Planning via Deep Reinforcement Learning](https://arxiv.org/abs/2506.08344)
...
### [374] [Re4MPC: Reactive Nonlinear MPC for Multi-model Motion Planning via Deep Reinforcement Learning](https://arxiv.org/abs/2506.08344)
*Neşet Ünver Akmandor,Sarvesh Prajapati,Mark Zolotas,Taşkın Padır*

Main category: cs.RO

TL;DR: 提出了一种名为Re4MPC的新型多模型运动规划管道，通过结合非线性模型预测控制（NMPC）和深度强化学习（DRL），高效生成机器人轨迹。
...
Method: Re4MPC通过DRL框架学习反应性选择NMPC模型、成本和约束的策略，并结合NMPC生成高效轨迹。
...
### [375] [Auto-Regressive vs Flow-Matching: a Comparative Study of Modeling Paradigms for Text-to-Music Generation](https://arxiv.org/abs/2506.08570)
...
### [376] [Auto-Regressive vs Flow-Matching: a Comparative Study of Modeling Paradigms for Text-to-Music Generation](https://arxiv.org/abs/2506.08570)
*Or Tal,Felix Kreuk,Yossi Adi*

Main category: cs.SD

TL;DR: 论文通过系统比较自回归解码和条件流匹配两种建模范式，分析了它们在文本到音乐生成中的表现，为未来系统设计提供指导。
...
Method: 使用相同数据集、训练配置和类似架构，对比自回归解码和条件流匹配两种范式，评估生成质量、鲁棒性等。

Result: 揭示了两种范式的优缺点，为未来系统设计提供了实用见解。

Conclusion: 研究为文本到音乐生成系统的建模范式选择提供了依据，并指出了未来研究方向。
...
### [377] [Generalizing while preserving monotonicity in comparison-based preference learning models](https://arxiv.org/abs/2506.08616)
...
### [378] [Generalizing while preserving monotonicity in comparison-based preference learning models](https://arxiv.org/abs/2506.08616)
*Julien Fageot,Peva Blanchard,Gilles Bareilles,Lê-Nguyên Hoang*

Main category: math.ST

TL;DR: 论文提出了一种新的线性广义Bradley-Terry模型，解决了现有偏好学习模型缺乏单调性保证的问题，并通过实验验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 现有基于比较的偏好学习模型（包括大语言模型）缺乏单调性保证，且唯一被证明单调的广义Bradley-Terry模型无法泛化到未比较数据。

Method: 提出了一种新的线性广义Bradley-Terry模型，结合扩散先验，并确定了保证单调性的嵌入条件。

Result: 实验表明，新模型在有限数据集上提高了准确性，且单调性并非普遍保证。

Conclusion: 新模型在保证单调性的同时具有泛化能力，尤其在数据有限时表现更优。
...
### [379] [Aligning Proteins and Language: A Foundation Model for Protein Retrieval](https://arxiv.org/abs/2506.08023)
...
### [380] [Aligning Proteins and Language: A Foundation Model for Protein Retrieval](https://arxiv.org/abs/2506.08023)
*Qifeng Wu,Zhengzhe Liu,Han Zhu,Yizhou Zhao,Daisuke Kihara,Min Xu*

Main category: q-bio.BM

TL;DR: 提出一种基于CLIP风格的框架，通过对比学习对齐3D蛋白质结构与功能注释，用于大规模蛋白质数据集中检索相似结构和语义的蛋白质。


<details>
  <summary>Details</summary>
Motivation: 近年来视觉-语言模型（VLMs）的进展为蛋白质结构与功能注释的对齐提供了新思路，旨在促进结构解析方法（如冷冻电镜）得到的蛋白质结构的功能解释。

Method: 采用CLIP风格的框架，利用对比学习对齐3D蛋白质结构与功能注释，并构建了一个包含约20万蛋白质-描述对的大规模数据集用于模型训练。

Result: 在蛋白质数据库（PDB）和电子显微镜数据库（EMDB）上分别进行了域内和跨数据库检索评估，模型在零样本检索任务中表现出色。

Conclusion: 多模态基础模型在蛋白质生物学中具有潜力，能够有效支持结构与功能的理解。
...
### [381] [A Comprehensive Study of Decoder-Only LLMs for Text-to-Image Generation](https://arxiv.org/abs/2506.08210)
...
### [382] [Seeing Voices: Generating A-Roll Video from Audio with Mirage](https://arxiv.org/abs/2506.08279)
...
### [383] [Diversity-Guided MLP Reduction for Efficient Large Vision Transformers](https://arxiv.org/abs/2506.08591)
...
### [384] [Landsat-Bench: Datasets and Benchmarks for Landsat Foundation Models](https://arxiv.org/abs/2506.08780)
...
### [385] [StreamSplat: Towards Online Dynamic 3D Reconstruction from Uncalibrated Video Streams](https://arxiv.org/abs/2506.08862)
...
### [386] [Segment Concealed Objects with Incomplete Supervision](https://arxiv.org/abs/2506.08955)
...
### [387] [Data Augmentation For Small Object using Fast AutoAugment](https://arxiv.org/abs/2506.08956)
...
### [388] [Efficient Medical Vision-Language Alignment Through Adapting Masked Vision Models](https://arxiv.org/abs/2506.08990)
...
### [389] [DIsoN: Decentralized Isolation Networks for Out-of-Distribution Detection in Medical Imaging](https://arxiv.org/abs/2506.09024)
...
### [390] [Diffuse and Disperse: Image Generation with Representation Regularization](https://arxiv.org/abs/2506.09027)
...
### [391] [A Comprehensive Study of Decoder-Only LLMs for Text-to-Image Generation](https://arxiv.org/abs/2506.08210)
*Andrew Z. Wang,Songwei Ge,Tero Karras,Ming-Yu Liu,Yogesh Balaji*
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型仍使用过时的T5和CLIP作为文本编码器，希望通过现代LLM提升性能。

Method: 构建标准化训练与评估流程，训练27个模型，分析12种文本编码器的嵌入提取方式、变体和模型大小。

Result: 多层级平均嵌入显著提升复杂提示的对齐性，多数LLM性能优于T5基线。

Conclusion: 现代LLM作为文本编码器在文本到图像生成中表现更优，尤其适用于复杂视觉语言推理。
...
### [392] [Seeing Voices: Generating A-Roll Video from Audio with Mirage](https://arxiv.org/abs/2506.08279)
*Aditi Sundararaman,Amogh Adishesha,Andrew Jaegle,Dan Bigioi,Hyoung-Kyu Song,Jon Kyl,Justin Mao,Kevin Lan,Mojtaba Komeili,ShahRukh Athar,Sheila Babayan,Stanislau Beliasau,William Buchwalter*

Main category: cs.CV

TL;DR: Mirage是一种音频到视频的基础模型，能够根据音频输入生成逼真、富有表现力的视频，适用于多模态视频生成。


<details>
  <summary>Details</summary>
Motivation: 视频的音频和图像序列的和谐结合对视频效果至关重要，但现有方法要么忽略声音，要么局限于特定领域。Mirage旨在解决这一问题。

Method: 提出了一种统一的训练方法，基于自注意力机制，支持从零开始训练或利用现有权重，生成高质量视频。

Result: Mirage生成的视频在主观质量上优于其他方法，能够逼真地表现音频中的表演内容。
...
### [393] [Diversity-Guided MLP Reduction for Efficient Large Vision Transformers](https://arxiv.org/abs/2506.08591)
*Chengchao Shen,Hourun Zhu,Gongfan Fang,Jianxin Wang,Xinchao Wang*
...
<details>
  <summary>Details</summary>
Motivation: 大型Transformer模型参数过多导致计算和内存成本高昂，MLP模块占用了大部分参数。

Method: 采用Gram-Schmidt权重剪枝策略，消除MLP隐藏层的冗余神经元，同时保留权重多样性以在蒸馏过程中恢复性能。

Result: 在多个大型视觉Transformer上，DGMR实现了57%以上的参数和FLOPs减少，性能损失极小；在EVA-CLIP-E（4.4B）上，参数和FLOPs减少71.5%且无性能下降。

Conclusion: DGMR是一种高效的模型压缩方法，显著降低资源需求，同时保持性能，适用于大规模视觉Transformer。
...
### [394] [Landsat-Bench: Datasets and Benchmarks for Landsat Foundation Models](https://arxiv.org/abs/2506.08780)
*Isaac Corley,Lakshay Sharma,Ruth Crasto*

Main category: cs.CV

TL;DR: Landsat-Bench是一套基于Landsat影像的基准测试，用于评估地理空间基础模型（GFM）的性能。


<details>
  <summary>Details</summary>
Motivation: Landsat数据缺乏标准化的基准测试，限制了基于Landsat的地理空间基础模型的发展。

Method: 引入三个基准测试（EuroSAT-L、BigEarthNet-L和LC100-L），并使用SSL4EO-L预训练的GFM与常见架构进行对比。

Result: SSL4EO-L预训练的GFM在下游任务中表现优于ImageNet，性能提升分别为+4% OA和+5.1% mAP。

Conclusion: Landsat-Bench为Landsat数据的标准化评估提供了工具，并验证了SSL4EO-L预训练模型的有效性。
...
### [395] [StreamSplat: Towards Online Dynamic 3D Reconstruction from Uncalibrated Video Streams](https://arxiv.org/abs/2506.08862)
*Zike Wu,Qi Yan,Xuanyu Yi,Lele Wang,Renjie Liao*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时处理未校准输入的实时性、动态场景建模的准确性以及长期稳定性和计算效率。

Method: 提出StreamSplat框架，包含静态编码器中的概率采样机制和动态解码器中的双向变形场。
...
### [396] [Segment Concealed Objects with Incomplete Supervision](https://arxiv.org/abs/2506.08955)
*Chunming He,Kai Li,Yachao Zhang,Ziyun Yang,Youwei Pang,Longxiang Tang,Chengyu Fang,Yulun Zhang,Linghe Kong,Xiu Li,Sina Farsiu*

Main category: cs.CV

TL;DR: 论文提出了一种统一方法SEE，用于不完全监督的隐蔽物体分割（ISCOS），通过结合SAM生成伪标签和混合粒度特征分组模块，解决了不完全监督和隐蔽物体与背景相似性的挑战。


<details>
  <summary>Details</summary>
Motivation: 隐蔽物体分割任务面临不完全标注数据和物体与背景高度相似的双重挑战，现有方法难以有效解决。

Method: 提出SEE框架，利用SAM生成伪标签，并通过伪标签生成、存储和监督策略优化训练；设计混合粒度特征分组模块提升分割一致性。

Result: 实验表明，SEE在多个ISCOS任务中达到最先进性能，并可作为即插即用方案提升现有模型。

Conclusion: SEE通过统一方法有效解决了ISCOS的核心挑战，具有实用性和推广性。
...
### [397] [Data Augmentation For Small Object using Fast AutoAugment](https://arxiv.org/abs/2506.08956)
*DaeEun Yoon,Semin Kim,SangWook Yoo,Jongha Lee*
...
<details>
  <summary>Details</summary>
Motivation: 尽管目标检测整体性能有显著进步，但小目标检测性能仍远低于大目标，这是计算机视觉中的重要挑战。

Method: 采用Fast AutoAugment快速寻找最优数据增强策略，以克服小目标检测中的性能下降问题。

Result: 在DOTA数据集上实现了20%的性能提升。

Conclusion: 提出的数据增强方法有效提升了小目标检测性能，为解决这一重要问题提供了新思路。
...
### [398] [Efficient Medical Vision-Language Alignment Through Adapting Masked Vision Models](https://arxiv.org/abs/2506.08990)
*Chenyu Lian,Hong-Yu Zhou,Dongyun Liang,Jing Qin,Liansheng Wang*
...
<details>
  <summary>Details</summary>
Motivation: 传统的跨模态对比学习方法在视觉表示能力上表现不佳，而多模态掩码建模方法虽在视觉表示上表现优异，但在跨模态匹配上效果有限。ALTA旨在解决这一矛盾。

Method: ALTA通过适应预训练的视觉模型（来自掩码记录建模），仅需约8%的可训练参数和不到1/5的计算消耗，并结合时间多视图放射影像输入增强信息一致性。

Result: ALTA在文本-图像准确率和图像-文本检索准确率上分别比最佳对比方法高出4%和6%。
...
### [399] [DIsoN: Decentralized Isolation Networks for Out-of-Distribution Detection in Medical Imaging](https://arxiv.org/abs/2506.09024)
*Felix Wagner,Pramit Saha,Harry Anthony,J. Alison Noble,Konstantinos Kamnitsas*

Main category: cs.CV

TL;DR: 论文提出了一种去中心化的OOD检测框架DIsoN，通过交换模型参数而非数据，解决了训练数据隐私和存储问题，并在医学影像数据上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 在安全关键领域（如医学影像）部署ML模型时，需要检测训练数据之外的输入（OOD），但现有方法因数据隐私或存储限制无法直接比较训练和测试数据。

Method: 提出了Isolation Network框架，通过二元分类任务量化测试样本与训练数据的分离难度，并进一步扩展为DIsoN，支持去中心化比较。
...
### [400] [Diffuse and Disperse: Image Generation with Representation Regularization](https://arxiv.org/abs/2506.09027)
*Runqian Wang,Kaiming He*

Main category: cs.CV

TL;DR: 提出了一种名为Dispersive Loss的简单插件式正则化方法，用于改进扩散生成模型，无需额外数据或预训练。


<details>
  <summary>Details</summary>
Motivation: 扩散生成模型通常缺乏显式正则化，且与表示学习的进展独立发展。本文旨在通过Dispersive Loss弥合生成模型与表示学习之间的差距。

Method: 提出Dispersive Loss，鼓励隐空间中的表示分散，类似于对比自监督学习，但无需正样本对，不影响回归采样过程。
...
### [403] [Inverse Design in Distributed Circuits Using Single-Step Reinforcement Learning](https://arxiv.org/abs/2506.08029)
*Jiayu Li,Masood Mortazavi,Ning Yan,Yihong Ma,Reza Zafarani*
...
Result: 实验表明，DCIDA的Transformer策略网络显著降低设计误差，尤其在复杂传输函数场景表现更优。
...
TL;DR: 论文提出了一种基于CNN和MLP的替代模型，用于近似2D壁域中参与性气体的辐射热传递解，以减少计算成本。CNN在精度和稳定性上优于MLP。


<details>
  <summary>Details</summary>
Motivation: 降低数值模拟的计算成本，同时保持工业可接受的精度。

Method: 使用CNN和MLP构建替代模型，通过Optuna优化超参数，并与传统求解器ICARUS2D进行比较。

Result: CNN和MLP均显著加速计算且误差可接受，CNN在精度和稳定性上优于MLP。
...
### [405] [Real-Time Cascade Mitigation in Power Systems Using Influence Graph Improved by Reinforcement Learning](https://arxiv.org/abs/2506.08893)
...
### [406] [Real-Time Cascade Mitigation in Power Systems Using Influence Graph Improved by Reinforcement Learning](https://arxiv.org/abs/2506.08893)
*Kai Zhou,Youbiao He,Chong Zhong,Yifu Wu*

Main category: physics.soc-ph

TL;DR: 本文提出了一种基于马尔可夫决策过程（MDP）的方法，用于实时缓解电力传输系统中的级联停电风险，结合强化学习快速生成保守决策。


<details>
  <summary>Details</summary>
Motivation: 现代电力系统虽可靠性高，但可再生能源渗透率增加导致级联停电风险上升，需快速应对不确定性。

Method: 将影响图扩展为MDP模型，结合强化学习（策略梯度算法）处理不确定性，并设计保守动作（如“不行动”）和奖励机制。

Result: 在IEEE 14和118总线系统验证中，主动断开线路可有效降低级联风险，某些线路被识别为关键。

Conclusion: 该方法能快速收敛并生成保守策略，为实时缓解级联停电提供有效工具。
